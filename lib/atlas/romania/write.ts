...
import type { RomaniaProjection, SqlRow } from "./project";

const ROMANIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearRomaniaProjection(db: DatabaseSync): void {
  for (const table of ROMANIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeRomaniaProjection(
  db: DatabaseSync,
  projection: RomaniaProjection,
  attemptId: string,
