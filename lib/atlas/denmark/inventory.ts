...
export type DenmarkOfficeRow = {
  ...
  next_election?: {
    ...
    precision: string;
    certainty: string;
    source?: DenmarkEvidenceRef | DenmarkEvidenceRef[];
    evidence?: DenmarkEvidenceRef | DenmarkEvidenceRef[];
    year?: number;
    month?: number;
    day?: number;
...
export type DenmarkUnresolvedRow = {
...
export type DenmarkInventory = {
  ...
  unresolved: DenmarkUnresolvedRow[];
  gaps: DenmarkUnresolvedRow[];
...
function loadSourceCatalogue(root: string, byPath: Map<string, TrackedInput>): DenmarkSourceRow[] {
  const landed = byPath.get(SOURCES_RELATIVE);
  if (landed) {
    return readJsonArray<DenmarkSourceRow>(landed.absPath, SOURCES_RELATIVE);
  }
  const lookupAbs = path.join(root, CATALOGUE_LOOKUP_PATH);
  if (!existsSync(lookupAbs)) {
    throw new Error(`Denmark source catalogue lookup is missing at ${CATALOGUE_LOOKUP_PATH}`);
  }
  const parsed = JSON.parse(readFileSync(lookupAbs, "utf8")) as { sources?: DenmarkSourceRow[] };
...
export function scanDenmarkInventory(options: {
  ...
  for (const rel of packagePaths) {
    ...
    try {
      ...
      const item: TrackedInput = {
        ...
        absPath: abs,
        text: logical.endsWith(".json") && bytes.length < 20_000_000 ? bytes.toString("utf8") : null,
      };
      tracked.push(item);
  ...
  if (sourceCatalogue.length !== EXPECTED_COUNTS.catalogue_rows) {
    throw new DenmarkPreflightError(
      "source_catalogue",
      `Expected ${EXPECTED_COUNTS.catalogue_rows} documented Denmark sources, found ${sourceCatalogue.length}.`,
      intendedInventory,
    );
  }
  ...
  return {
    ...
    sourceCatalogue,
    unresolved: readJsonArray<DenmarkUnresolvedRow>(byPath.get(UNRESOLVED_RELATIVE)!.absPath, UNRESOLVED_RELATIVE),
    gaps: readJsonArray<DenmarkUnresolvedRow>(byPath.get(GAPS_RELATIVE)!.absPath, GAPS_RELATIVE),
