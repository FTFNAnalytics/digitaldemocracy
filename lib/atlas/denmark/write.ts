...
import type { DenmarkProjection, SqlRow } from "./project";

const DENMARK_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearDenmarkProjection(db: DatabaseSync): void {
  for (const table of DENMARK_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeDenmarkProjection(
  db: DatabaseSync,
  projection: DenmarkProjection,
  attemptId: string,
