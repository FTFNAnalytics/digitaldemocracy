...
function parseDayLabel(label: string): { year: number; month: number; day: number } {
  ...
  if (!match) throw new Error(`Denmark event date is not a valid ISO day: ${JSON.stringify(label)}`);
...
function pointerOrLocator(ref: DenmarkEvidenceRef): string {
...
export function projectDenmark(inventory: DenmarkInventory): DenmarkProjection {
  ...
  if (inventory.proceedings.length !== 0) {
    throw new Error("Denmark proceedings.json must be empty");
  }
    ...
    raw_json: rawEnvelope({
      ...
      supplemental: {
        ...
        open_notes: [
          ...
          "ep_detail_gaps",
          "no_popular_mayor_rows_council_elected_borgmester",
        ],
      },
    }),
  };
  ...
  const geographies: SqlRow[] = inventory.geographies.map((row, index) => {
    if (row.country_id !== COUNTRY_ID) throw new Error(`Geography ${row.geography_id} is not Denmark`);
  ...
  for (let i = 0; i < inventory.offices.length; i++) {
    ...
    if (row.country_id !== COUNTRY_ID) throw new Error(`Office ${row.office_id} country is not Denmark`);
    ...
    if (isMayorToken(row.office_id) || isMayorToken(row.office_type) || isMayorToken(row.name)) {
      mayorOffices += 1;
      throw new Error(`Denmark must not project popular mayor rows; found ${row.office_id}`);
    }
    ...
    const historicNote = historical
      ? "Historical coded jurisdiction retained; 2007/earlier merger successor review remains open. No inferred abolition date."
      : null;
  ...
  for (let i = 0; i < inventory.events.length; i++) {
    ...
    const yearOnly = row.precision === "year" || !row.date;
    const dateIdValue = dateId("election_event", row.event_id, "ballot");
    if (yearOnly) {
      if (row.year == null) throw new Error(`Year-precision event ${row.history_key} is missing year`);
      eventDatesYear += 1;
      pushDate({
        date_id: dateIdValue,
        label: String(row.year),
        precision: "year",
        certainty: row.certainty ?? "called",
        year: row.year,
        month: null,
        day: null,
        range_start_id: null,
        ...
        release_id: R,
        raw_json: rawEnvelope({
          origin: originFor("data/research/denmark/events.json", eventsHash, i, "year"),
          row: { date: row.date ?? null, year: row.year, precision: row.precision, certainty: row.certainty },
        }),
      });
    } else {
      ...
      pushDate({
        ...
        raw_json: rawEnvelope({
          ...
          row: { date: row.date, date_precision: row.precision, date_certainty: row.certainty },
        }),
      });
    }
    ...
    events.push({
      ...
      record_state: "active",
      state_note: row.raw ? canonical(row.raw) : null,
      lineage_id: L,
      release_id: R,
      raw_json: rawEnvelope({ origin, row }),
    });
  ...
  const sourcesHash = sliceHash(inventory, "data/research/denmark/sources.json");
  for (let i = 0; i < inventory.sourceCatalogue.length; i++) {
    const row = inventory.sourceCatalogue[i]!;
    if (!row.source_id || !row.input_path) {
      throw new Error(`Denmark source catalogue row ${i} is missing source_id/input_path`);
    }
    sourceByPath.set(row.input_path, row.source_id);
    if (sourceById.has(row.source_id)) {
      throw new Error(`Duplicate Denmark source_id ${row.source_id}`);
    }
    sourceById.add(row.source_id);
    const origin = originFor("data/research/denmark/sources.json", sourcesHash, i);
    sources.push({
      country_id: COUNTRY_ID,
  ...
  for (let i = 0; i < inventory.results.length; i++) {
    ...
    if (isMayorToken(row.office_id) || isMayorToken(row.candidate_or_list_label ?? null)) {
      // Candidate labels may mention a kommune; only reject office-level mayor tokens.
    }
    ...
    else if (row.row_type === "elected_candidate") resultCandidate += 1;
  ...
  for (const [index, row] of inventory.offices.entries()) {
    ...
    if (row.office_id === COPENHAGEN_COUNCIL_ID && rec !== COPENHAGEN_RECORD_KEY) {
  ...
  for (const row of sources) {
    addLocator(locators, locatorSeen, {
      ...
      release_id: R,
      source_row_locator: sourceRowLocator({
        inputPath: "data/research/denmark/sources.json",
        sha256: sourcesHash,
  ...
  const pushEvidence = (args: {
    ...
    for (const ref of args.refs ?? []) {
      ...
      const evidenceId = denmarkEvidenceId(args.recordKey, sourceId, ref.input_path, occurrence, args.claimKind);
  ...
  for (const event of events) {
    ...
    pushEvidence({
      ...
      claim: { history_key: event.history_key, date: source.date ?? source.year ?? null },
    });
  }
  for (const [index, row] of inventory.results.entries()) {
    pushEvidence({
  ...
  const gapsHash = sliceHash(inventory, "data/research/denmark/research-gaps.json");
  for (let i = 0; i < inventory.researchGaps.length; i++) {
    const row = inventory.researchGaps[i]!;
    const origin = originFor("data/research/denmark/research-gaps.json", gapsHash, i);
    const sourceLocator = canonical({
      input_path: "data/research/denmark/research-gaps.json",
      json_pointer: `/${i}`,
    });
    unresolved.push({
      unresolved_id: denmarkUnresolvedId(countryRec, sourceLocator, row.original_token),
      record_key: countryRec,
  ...
  const unresolvedHash = sliceHash(inventory, "data/research/denmark/unresolved-candidate-bindings.json");
  for (let i = 0; i < inventory.unresolvedCandidates.length; i++) {
    const row = inventory.unresolvedCandidates[i]!;
    const origin = originFor("data/research/denmark/unresolved-candidate-bindings.json", unresolvedHash, i);
    const sourceLocator = canonical({
      input_path: "data/research/denmark/unresolved-candidate-bindings.json",
      json_pointer: `/${i}`,
    });
    const token = row.candidate_code != null
      ? `${row.table ?? "candidate"}:${row.candidate_code}`
      : row.original_label ?? row.reason;
    unresolved.push({
      unresolved_id: denmarkUnresolvedId(countryRec, sourceLocator, token),
      record_key: countryRec,
  ...
  const validatedCounts = {
    ...
    result_rows_elected_candidate: resultCandidate,
    ...
    unresolved_candidate_bindings: inventory.unresolvedCandidates.length,
    ...
    next_dates_day_statutory: nextDates,
    proceedings: 0,
    party_mappings: 0,
  ...
  const lineage = {
    ...
    description:
      "Denmark current-register and historic primary-source research; coverage partial. Prompt X accepted 106 current + 240 historical offices. Greenland/Faroe Realm systems remain separately gated.",
  };
  const release = {
    lineage_id: L,
    release_id: R,
