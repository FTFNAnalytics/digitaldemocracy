...
export type AustriaInventory = {
  ...
  companionBriefings: WorkbookSlice;
  pollingEvidence: WorkbookSlice;
  tiers: {
    status: string;
...
function walkRegularFiles(dir: string, relBase: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink() || lstatSync(abs).isSymbolicLink()) {
      throw new Error(`Unexpected symlink in Austria package: ${rel}`);
    }
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unexpected non-file in Austria package: ${rel}`);
    }
  }
  return out.sort();
}
...
export class AustriaPreflightError extends Error {
...
function extractAustriaUstar(buffer: Buffer): Map<string, Buffer> {
  ...
  while (offset + 512 <= buffer.length) {
    ...
    if (typeflag === "1" || typeflag === "2") {
      throw new Error(`Austria payload rejects links: ${full}`);
    }
...
function xzDecompress(buffer: Buffer): Buffer {
  try {
    ...
    throw new Error(`Austria payload is not valid xz: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export function unpackAustriaMembers(packageDir: string): Map<string, Buffer> {
  const manifestPath = path.join(packageDir, "manifest.json");
  if (!existsSync(manifestPath)) {
    throw new Error("Austria manifest.json is missing.");
  }
  ...
  for (const [relative, expected] of Object.entries(manifest.chunks ?? {})) {
    ...
    if (!existsSync(abs)) {
      throw new Error(`Missing Austria payload chunk ${relative}`);
    }
    const bytes = readFileSync(abs);
    if (bytes.length !== expected.bytes || adapterSha256(bytes) !== expected.sha256) {
      throw new Error(`Austria payload chunk mismatch: ${relative}`);
    }
    parts.push(bytes);
  }
  const xz = Buffer.concat(parts);
  if (adapterSha256(xz) !== manifest.payload_sha256) {
    throw new Error("Austria concatenated payload checksum mismatch");
  }
  const files = extractAustriaUstar(xzDecompress(xz));
  if (files.size !== EXPECTED_COUNTS.payload_member_files) {
    throw new Error(`Expected ${EXPECTED_COUNTS.payload_member_files} Austria payload members, found ${files.size}`);
  }
  const inventoryBytes = files.get("inventory.json");
  if (!inventoryBytes) {
    throw new Error("Austria payload is missing inventory.json");
  }
  ...
  for (const [entry, expected] of Object.entries(contents)) {
    ...
    if (bytes.length !== expected.bytes || adapterSha256(bytes) !== expected.sha256) {
      throw new Error(`Austria member hash mismatch: ${entry}`);
    }
  }
  for (const entry of files.keys()) {
    if (entry === "inventory.json") continue;
    if (!contents[entry]) {
      throw new Error(`Unexpected Austria payload member ${entry}`);
    }
  }
  return files;
}

export function scanAustriaInventory(options: {
  root: string;
  packageDir?: string;
  tierPath?: string;
  ...
  if (sha256Hex(readFileSync(schemaAttempt)) !== ATTEMPT_LOG_SHA256) {
    throw new AustriaPreflightError("schema_hash_mismatch", "Attempt-log SQL bytes do not match the Identity Rules digest.", {});
  }
  if (sha256Hex(readFileSync(schemaMaster)) !== MASTER_SCHEMA_SHA256) {
    throw new AustriaPreflightError("schema_hash_mismatch", "Master SQL bytes do not match the Identity Rules digest.", {});
  }
  ...
  if (requireGit) {
    ...
    if (tracked.length !== EXPECTED_COUNTS.outer_package_files) {
      throw new AustriaPreflightError(
        "package_inventory",
        `Expected ${EXPECTED_COUNTS.outer_package_files} git-tracked Austria files, found ${tracked.length}.`,
        {},
      );
    }
    if (JSON.stringify(tracked) !== JSON.stringify(walked)) {
      throw new AustriaPreflightError(
        "unpinned_worktree",
        "Austria package worktree files do not match git-tracked paths.",
        {},
      );
    }
  ...
  if (failedFiles.length > 0) {
    throw new AustriaPreflightError(
      "package_read",
      `Failed to read Austria package files: ${failedFiles.map((item) => item.input_path).join(", ")}`,
  ...
  try {
    members = unpackAustriaMembers(packageDir);
  } catch (error) {
    throw new AustriaPreflightError(
      "packed_input",
  ...
  if (tierMeta.error === "missing_tier_file") {
    throw new AustriaPreflightError(
      "missing_tier",
      `Approved Austria tier file is missing at ${TIER_PATH}.`,
      intendedInventory,
    );
  }

  let tierJson: AustriaInventory["tiers"];
  try {
    tierJson = JSON.parse(tierBytes!.toString("utf8")) as AustriaInventory["tiers"];
  } catch (error) {
    throw new AustriaPreflightError(
      "tier_unreadable",
      `Austria tier file is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      intendedInventory,
    );
  }
  ...
  if (tierJson.status !== "approved") {
    throw new AustriaPreflightError(
      "tier_not_approved",
      `Austria tiers status is ${JSON.stringify(tierJson.status)}; import requires status "approved".`,
      intendedInventory,
    );
  }
  if (tierMeta.sha256 !== TIER_SHA256) {
    throw new AustriaPreflightError(
      "tier_hash_mismatch",
      `Approved Austria tier SHA-256 mismatch; expected ${TIER_SHA256}.`,
      intendedInventory,
    );
  }
  ...
  if (!manifestItem?.text) {
    throw new AustriaPreflightError("missing_manifest", "Austria manifest.json is missing.", intendedInventory);
  }
  const manifest = JSON.parse(manifestItem.text) as Record<string, unknown>;
  if (manifest.country !== "Austria") {
    throw new AustriaPreflightError("manifest_country", "Austria manifest country mismatch.", intendedInventory);
  }

  const coverageItem = byPath.get(`${PACKAGE_PREFIX}/coverage.json`);
  if (!coverageItem?.text) {
    throw new AustriaPreflightError("missing_coverage", "Austria coverage.json is missing.", intendedInventory);
  }
  const sourceLinksItem = byPath.get(`${UNPACKED_PREFIX}/source-links.json`);
  if (!sourceLinksItem?.text) {
    throw new AustriaPreflightError("missing_source_links", "Austria source-links.json is missing.", intendedInventory);
  }
  ...
  const memberFile = (entry: string) => {
    ...
    if (!item) throw new AustriaPreflightError("missing_input", `Missing ${logical}`, intendedInventory);
  ...
  if (!historyIndexItem?.text) {
    throw new AustriaPreflightError("missing_input", "Missing history-index.json", intendedInventory);
  }
  const historyIndexRows = JSON.parse(historyIndexItem.text) as Array<Record<string, unknown>>;
  if (!Array.isArray(historyIndexRows)) {
    throw new AustriaPreflightError("packed_input", "history-index.json must be an array", intendedInventory);
  }
  ...
  return {
    ...
    sourceLinks: JSON.parse(sourceLinksItem.text) as AustriaInventory["sourceLinks"],
    officeRegister: memberFile("tables/master/office-register.json"),
    jurisdictions: memberFile("tables/companion/jurisdictions.json"),
    histories: memberFile("tables/companion/histories.json"),
    historyIndex: {
      relativePath: `${UNPACKED_PREFIX}/history-index.json`,
      archiveEntry: "history-index.json",
      rows: historyIndexRows,
    },
    returns: memberFile("tables/companion/full-results.json"),
    ...
    companionBriefings: memberFile("tables/companion/briefings.json"),
    pollingEvidence: memberFile("tables/master/polling-evidence.json"),
    tiers: tierJson,
    intendedInventory,
  };
}
