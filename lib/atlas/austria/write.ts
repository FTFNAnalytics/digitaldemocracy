...
import type { AustriaProjection, SqlRow } from "./project";

const AUSTRIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearAustriaProjection(db: DatabaseSync): void {
  for (const table of AUSTRIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeAustriaProjection(
  db: DatabaseSync,
  projection: AustriaProjection,
  attemptId: string,
