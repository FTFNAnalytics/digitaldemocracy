...
import type { FranceProjection, SqlRow } from "./project";

const FRANCE_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearFranceProjection(db: DatabaseSync): void {
  for (const table of FRANCE_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeFranceProjection(
  db: DatabaseSync,
