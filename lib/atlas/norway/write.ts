...
import type { NorwayProjection, SqlRow } from "./project";

const NORWAY_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearNorwayProjection(db: DatabaseSync): void {
  for (const table of NORWAY_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeNorwayProjection(
  db: DatabaseSync,
  projection: NorwayProjection,
  attemptId: string,
