...
export type NorwayAuthoredDate = {
  value: string;
  precision: string;
  certainty: string;
  year?: number;
  ...
  day?: number;
  conflicting?: boolean;
  evidence?: NorwayEvidenceRef[];
};

export type NorwayOfficeRow = {
  id_namespace: string;
  office_id: string;
  country_id: string;
  geography_id: string;
  ...
  next_election?: NorwayAuthoredDate | null;
  evidence?: NorwayEvidenceRef[];
  review_notes?: string[];
  notes?: string[];
...
export type NorwayEventRow = {
  ...
  date?: NorwayAuthoredDate | string | null;
...
export type NorwayGeographyRow = {
  geography_id: string;
  name: string;
  country_id: string;
...
export type NorwayResearchGapRow = {
...
export type NorwayReformClaimRow = {
  kind?: string;
  claim?: unknown;
  evidence?: NorwayEvidenceRef[];
  atlas_successor_asserted?: boolean;
  code_alias_eligible?: boolean;
  [key: string]: unknown;
};

export type NorwayCrosswalkRow = {
...
export type NorwayTierClassification = {
  office_id: string;
  tier: string;
...
export type NorwayInventory = {
  ...
  offices: NorwayOfficeRow[];
  events: NorwayEventRow[];
  results: NorwayResultRow[];
  geographies: NorwayGeographyRow[];
  sourceCatalogue: NorwaySourceRow[];
  researchGaps: NorwayResearchGapRow[];
  crosswalks: NorwayCrosswalkRow[];
  reformClaims: NorwayReformClaimRow[];
  proceedings: unknown[];
  tiers: {
    status: string;
    production_accepted?: boolean;
    classifications: NorwayTierClassification[];
...
function walkRegularFiles(dir: string, relBase: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink() || lstatSync(abs).isSymbolicLink()) {
      throw new Error(`Unexpected symlink in Norway research pack: ${rel}`);
    }
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unexpected non-file in Norway research pack: ${rel}`);
    }
  }
  return out.sort();
}
...
export class NorwayPreflightError extends Error {
...
export function scanNorwayInventory(options: {
  root: string;
  researchDir?: string;
  tierPath?: string;
  ...
  if (sha256Hex(readFileSync(schemaAttempt)) !== ATTEMPT_LOG_SHA256) {
    throw new NorwayPreflightError("schema_hash_mismatch", "Attempt-log SQL bytes do not match the Identity Rules digest.", {});
  }
  if (sha256Hex(readFileSync(schemaMaster)) !== MASTER_SCHEMA_SHA256) {
    throw new NorwayPreflightError("schema_hash_mismatch", "Master SQL bytes do not match the Identity Rules digest.", {});
  }
  ...
  if (requireGit) {
    ...
    if (tracked.length !== EXPECTED_COUNTS.retained_inputs) {
      throw new NorwayPreflightError(
        "package_inventory",
        `Expected ${EXPECTED_COUNTS.retained_inputs} git-tracked Norway research/tier files, found ${tracked.length}.`,
        {},
      );
    }
    if (JSON.stringify(tracked) !== JSON.stringify(walked)) {
      throw new NorwayPreflightError(
        "unpinned_worktree",
        "Norway research worktree files do not match git-tracked paths.",
        {},
      );
    }
  ...
  if (tierMeta.error === "missing_tier_file" || !tierItem) {
    throw new NorwayPreflightError("missing_tier", `Approved Norway tier file is missing at ${TIER_PATH}.`, intendedInventory);
  }

  let tierJson: NorwayInventory["tiers"];
  try {
    tierJson = JSON.parse(tierItem.text ?? "") as NorwayInventory["tiers"];
  } catch (error) {
    throw new NorwayPreflightError(
      "tier_unreadable",
      `Norway tier file is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      intendedInventory,
    );
  }
  ...
  if (tierJson.status !== "approved" || tierJson.production_accepted !== true) {
    throw new NorwayPreflightError(
      "tier_not_approved",
      `Norway tiers status is ${JSON.stringify(tierJson.status)}; import requires approved production-accepted bytes.`,
      intendedInventory,
    );
  }
  if (tierItem.sha256 !== TIER_SHA256) {
    throw new NorwayPreflightError(
      "tier_hash_mismatch",
      `Norway tier SHA-256 mismatch; expected ${TIER_SHA256}.`,
      intendedInventory,
    );
  }
  ...
  if (tracked.length !== EXPECTED_COUNTS.retained_inputs) {
    throw new NorwayPreflightError(
      "package_inventory",
      `Expected ${EXPECTED_COUNTS.retained_inputs} retained Norway inputs, found ${tracked.length}.`,
      intendedInventory,
    );
  }
  ...
  for (const rel of mustRead) {
    if (!byPath.has(rel)) {
      throw new NorwayPreflightError("missing_research_table", `Missing required Norway research table ${rel}.`, intendedInventory);
    }
  }
  ...
  return {
    ...
    hashInputs,
    offices: readJsonArray<NorwayOfficeRow>(byPath.get(REGISTER_RELATIVE)!.absPath, REGISTER_RELATIVE),
    events: readJsonArray<NorwayEventRow>(byPath.get(EVENTS_RELATIVE)!.absPath, EVENTS_RELATIVE),
    results: readJsonArray<NorwayResultRow>(byPath.get(RESULTS_RELATIVE)!.absPath, RESULTS_RELATIVE),
    geographies: readJsonArray<NorwayGeographyRow>(byPath.get(GEOGRAPHY_RELATIVE)!.absPath, GEOGRAPHY_RELATIVE),
    sourceCatalogue: readJsonArray<NorwaySourceRow>(byPath.get(SOURCES_RELATIVE)!.absPath, SOURCES_RELATIVE),
    researchGaps: readJsonArray<NorwayResearchGapRow>(byPath.get(GAPS_RELATIVE)!.absPath, GAPS_RELATIVE),
    crosswalks: readJsonArray<NorwayCrosswalkRow>(byPath.get(CROSSWALK_RELATIVE)!.absPath, CROSSWALK_RELATIVE),
    reformClaims: readJsonArray<NorwayReformClaimRow>(byPath.get(REFORM_CLAIMS_RELATIVE)!.absPath, REFORM_CLAIMS_RELATIVE),
