...
function parseDayLabel(label: string): { year: number; month: number; day: number } {
  ...
  if (!match) throw new Error(`Norway event date is not a valid ISO day: ${JSON.stringify(label)}`);
...
function parseYearLabel(label: string): number {
  ...
  if (!match) throw new Error(`Norway year date is not YYYY: ${JSON.stringify(label)}`);
  return Number(match[1]);
}

function authoredDate(value: NorwayAuthoredDate | string | null | undefined): NorwayAuthoredDate | null {
  ...
  if (!value.value || !value.precision) {
    throw new Error(`Norway authored date is missing value/precision: ${JSON.stringify(value)}`);
  }
  return value;
}
...
function sliceHash(inventory: NorwayInventory, relativePath: string): string {
...
function mapTier(classification: { tier: string; schema_v1_tier?: string }): string | null {
  ...
  throw new Error(`Unsupported Norway classification tier ${JSON.stringify(raw)}`);
}
...
function pointerOrLocator(ref: NorwayEvidenceRef): string {
...
export function projectNorway(inventory: NorwayInventory): NorwayProjection {
  const L = LINEAGE_ID;
  ...
  if (inventory.offices.length !== EXPECTED_COUNTS.offices) {
    throw new Error(`Expected ${EXPECTED_COUNTS.offices} Norway offices, found ${inventory.offices.length}`);
  }
  if (inventory.events.length !== EXPECTED_COUNTS.total_events) {
    throw new Error(`Expected ${EXPECTED_COUNTS.total_events} Norway events, found ${inventory.events.length}`);
  }
  if (inventory.results.length !== EXPECTED_COUNTS.result_rows) {
    throw new Error(`Expected ${EXPECTED_COUNTS.result_rows} Norway results, found ${inventory.results.length}`);
  }
  if (inventory.geographies.length !== EXPECTED_COUNTS.geographies) {
    throw new Error(`Expected ${EXPECTED_COUNTS.geographies} Norway geographies, found ${inventory.geographies.length}`);
  }
  if (inventory.tiers.classifications.length !== EXPECTED_COUNTS.offices) {
    throw new Error(`Expected ${EXPECTED_COUNTS.offices} Norway classifications`);
  }
  if (inventory.proceedings.length !== 0) {
    throw new Error("Norway proceedings.json must be empty");
  }
  if (inventory.researchGaps.length !== EXPECTED_COUNTS.unresolved_research_gaps) {
    throw new Error(`Expected ${EXPECTED_COUNTS.unresolved_research_gaps} Norway named holds, found ${inventory.researchGaps.length}`);
  }
  const gapTokens = inventory.researchGaps.map((row) => row.original_token);
  for (const hold of NAMED_HOLDS) {
    if (!gapTokens.includes(hold)) {
      throw new Error(`Norway named hold ${hold} is missing from research-gaps.json`);
    }
  }
  ...
  const registerHash = sliceHash(inventory, "data/research/norway/office-register.json");
  ...
  const country: SqlRow = {
    ...
    raw_json: rawEnvelope({
      origin: locator({
        input_path: "data/research/norway/office-register.json",
        sha256: registerHash,
        json_pointer: "/0",
      }),
      row: {
  ...
  const geographies: SqlRow[] = inventory.geographies.map((row, index) => {
    if (row.country_id !== COUNTRY_ID) throw new Error(`Geography ${row.geography_id} is not Norway`);
    geoById.set(row.geography_id, row.geography_id);
    const origin = originFor(
      "data/research/norway/geographies.json",
      sliceHash(inventory, "data/research/norway/geographies.json"),
      index,
    );
    return {
      country_id: COUNTRY_ID,
  ...
  let nextDayCalled = 0;
  let mayorOffices = 0;
  let osloBoroughOffices = 0;
  let osloCountyOffices = 0;
  let epOffices = 0;

  for (let i = 0; i < inventory.offices.length; i++) {
    const row = inventory.offices[i]!;
    if (row.country_id !== COUNTRY_ID) throw new Error(`Office ${row.office_id} country is not Norway`);
    ...
    if (isMayorToken(row.office_id) || isMayorToken(row.office_type) || isMayorToken(row.name)) {
      mayorOffices += 1;
      throw new Error(`Norway must not project popular mayor/executive/EP rows; found ${row.office_id}`);
    }
    if (row.office_id === OSLO_COUNTY_OFFICE_ID || /^NO-F03-C$/.test(row.office_id)) {
      osloCountyOffices += 1;
      throw new Error("Norway must not invent a separate Oslo fylkesting office");
    }
    if (row.office_type === "borough_committee" || row.office_id.startsWith("NO-B")) {
      osloBoroughOffices += 1;
    }
    if (/^NO-EP/i.test(row.office_id) || /europaparlament/i.test(row.name) || /europaparlament/i.test(row.office_type)) {
      epOffices += 1;
      throw new Error(`Norway must not invent an EP office; found ${row.office_id}`);
    }
    if (row.successor_office_id) {
      throw new Error(`Norway must not invent successor_office_id on ${row.office_id}`);
    }
    ...
    if (next) {
      ...
      if (next.precision === "year") {
        ...
        if (next.certainty === "expected") nextYearExpected += 1;
        else throw new Error(`Unsupported Norway year next-date certainty ${JSON.stringify(next.certainty)} on ${row.office_id}`);
        pushDate({
          date_id: nextDateId,
          label: next.value,
          precision: "year",
        ...
        if (next.certainty !== "called") {
          throw new Error(`Norway day next-date must stay called; got ${JSON.stringify(next.certainty)} on ${row.office_id}`);
        }
        nextDayCalled += 1;
        pushDate({
          date_id: nextDateId,
        ...
        throw new Error(`Unsupported Norway next-election precision ${JSON.stringify(next.precision)} on ${row.office_id}`);
      }
    }
    const origin = originFor("data/research/norway/office-register.json", registerHash, i);
    offices.push({
      id_namespace: N,
  ...
  if (osloCountyOffices !== 0) {
    throw new Error("Norway must not invent a separate Oslo fylkesting office");
  }
  if (epOffices !== 0) {
    throw new Error("Norway must not invent an EP office");
  }
  ...
  const events: SqlRow[] = [];
  const eventByHk = new Map<string, NorwayEventRow>();
  let selectedHistories = 0;
  let otherHistories = 0;
  let prospectiveEvents = 0;
  let eventDatesDay = 0;
  let eventDatesYear = 0;
  let osloBoroughEvents = 0;
  const eventsHash = sliceHash(inventory, "data/research/norway/events.json");
  ...
  for (let i = 0; i < inventory.events.length; i++) {
    ...
    if (date.precision === "year") {
      ...
      pushDate({
        ...
        release_id: R,
        raw_json: rawEnvelope({
          origin: originFor("data/research/norway/events.json", eventsHash, i, "date"),
          row: date,
        }),
      });
      ...
      throw new Error(`Unsupported Norway event precision ${JSON.stringify(date.precision)} on ${row.history_key}`);
    }
    ...
    else throw new Error(`Unsupported Norway selected_history_role ${JSON.stringify(role)} for ${row.history_key}`);
    if (row.office_id.startsWith("NO-B")) {
      osloBoroughEvents += 1;
    }
    if (row.event_kind && row.event_kind !== "ordinary") {
      throw new Error(`Norway must not invent non-ordinary events; got ${row.event_kind} for ${row.history_key}`);
    }

    const origin = originFor("data/research/norway/events.json", eventsHash, i);
    events.push({
      id_namespace: N,
      office_id: row.office_id,
  ...
  const sourcesHash = sliceHash(inventory, "data/research/norway/sources.json");
  for (let i = 0; i < inventory.sourceCatalogue.length; i++) {
    const row = inventory.sourceCatalogue[i]!;
    if (!row.source_id || !row.input_path) {
      throw new Error(`Norway source catalogue row ${i} is missing source_id/input_path`);
    }
    sourceByPath.set(row.input_path, row.source_id);
    if (sourceById.has(row.source_id)) {
      throw new Error(`Duplicate Norway source_id ${row.source_id}`);
    }
    sourceById.add(row.source_id);
    const origin = originFor("data/research/norway/sources.json", sourcesHash, i);
    sources.push({
      country_id: COUNTRY_ID,
      ...
      url: row.url ?? null,
      checked_as_of_label: row.accessed_date ?? row.retrieved_on ?? row.retrieved_at ?? RESEARCH_SNAPSHOT_LABEL,
      evidence_grade: null,
  ...
  const resultsHash = sliceHash(inventory, "data/research/norway/results.json");
  let mayorResults = 0;
  let resultPartyCategory = 0;
  let longyearbyenResults = 0;
  let sami2025Disputed = 0;
  let reformSuccessorsAsserted = 0;
  for (let i = 0; i < inventory.results.length; i++) {
    ...
    if (row.office_id === LONGYEARBYEN_ID) longyearbyenResults += 1;
    if (row.result_row_id === SAMI_2025_DISPUTED_RESULT_ID) {
      sami2025Disputed += 1;
      if (row.votes !== 0 || Number(row.seats) !== 1 || row.evidence_status !== "disputed") {
        throw new Error("Sami 2025 98d disputed result must preserve zero votes and one seat");
      }
    }
    const origin = originFor("data/research/norway/results.json", resultsHash, i);
    results.push({
      id_namespace: N,
      office_id: row.office_id,
      history_key: row.history_key,
  ...
  if (mayorResults !== 0) throw new Error("Norway must not project mayor vote rows");
  if (longyearbyenResults !== 0) throw new Error("Longyearbyen result histories must not be invented");
  ...
  addLocator(locators, locatorSeen, {
    ...
    source_row_locator: sourceRowLocator({
      inputPath: "data/research/norway/office-register.json",
      sha256: registerHash,
      derivedPointer: "/0",
    }),
  });
  for (const [index, row] of inventory.geographies.entries()) {
    addLocator(locators, locatorSeen, {
      ...
      release_id: R,
      source_row_locator: sourceRowLocator({
        inputPath: "data/research/norway/geographies.json",
        sha256: sliceHash(inventory, "data/research/norway/geographies.json"),
  ...
  for (const [index, row] of inventory.offices.entries()) {
    ...
    if (row.office_id === OSLO_COUNCIL_ID && rec !== OSLO_RECORD_KEY) {
      throw new Error(`Oslo bystyre record_key drifted: ${rec}`);
    }
    addLocator(locators, locatorSeen, {
      record_key: rec,
      entity_kind: "office",
  ...
  for (const [index, row] of inventory.events.entries()) {
    addLocator(locators, locatorSeen, {
      ...
      release_id: R,
      source_row_locator: sourceRowLocator({
        inputPath: "data/research/norway/events.json",
        sha256: eventsHash,
        derivedPointer: `/${index}`,
      }),
    });
  ...
  for (const [index, row] of inventory.results.entries()) {
    addLocator(locators, locatorSeen, {
      ...
      release_id: R,
      source_row_locator: sourceRowLocator({
        inputPath: "data/research/norway/results.json",
        sha256: resultsHash,
        derivedPointer: `/${index}`,
      }),
    });
  ...
  for (const row of sources) {
    addLocator(locators, locatorSeen, {
      ...
      release_id: R,
      source_row_locator: sourceRowLocator({
        inputPath: "data/research/norway/sources.json",
        sha256: sourcesHash,
  ...
  const pushEvidence = (args: {
    recordKey: string;
    refs: NorwayEvidenceRef[] | undefined;
    claimKind: string;
    ...
    for (const ref of args.refs ?? []) {
      ...
      const evidenceId = norwayEvidenceId(args.recordKey, sourceId, ref.input_path, occurrence, args.claimKind);
  ...
  const gapsHash = sliceHash(inventory, "data/research/norway/research-gaps.json");
  for (let i = 0; i < inventory.researchGaps.length; i++) {
    const row = inventory.researchGaps[i]!;
    const origin = originFor("data/research/norway/research-gaps.json", gapsHash, i);
    const sourceLocator = canonical({
      input_path: "data/research/norway/research-gaps.json",
      json_pointer: `/${i}`,
    });
    unresolved.push({
      unresolved_id: norwayUnresolvedId(countryRec, sourceLocator, row.original_token),
      record_key: countryRec,
  ...
  for (const row of inventory.reformClaims) {
    if (row.atlas_successor_asserted === true) {
      reformSuccessorsAsserted += 1;
      throw new Error("Norway reform-source-claims must not assert Atlas successor links");
    }
  }
  ...
  const validatedCounts = {
    ...
    oslo_county_offices: osloCountyOffices,
    oslo_borough_offices: osloBoroughOffices,
    oslo_borough_events: osloBoroughEvents,
    longyearbyen_result_rows: longyearbyenResults,
    ep_offices: epOffices,
    sami_2025_disputed: sami2025Disputed,
    reform_successors_asserted: reformSuccessorsAsserted,
  };

  for (const [key, expected] of Object.entries(EXPECTED_COUNTS)) {
    if (validatedCounts[key as keyof typeof validatedCounts] !== expected) {
      throw new Error(`Norway ${key} count ${validatedCounts[key as keyof typeof validatedCounts]} != ${expected}`);
    }
  }
  ...
  const lineage = {
    ...
    description:
      "Norway current-register and historic primary-source research; coverage partial. Prompt AA accepted 389 current + 537 historical offices with named holds. Do not invent a separate Oslo fylkesting, merger successors, borough/Longyearbyen result histories, or EP offices.",
  };
  const release = {
    lineage_id: L,
    release_id: R,
