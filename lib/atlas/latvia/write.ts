...
import type { LatviaProjection, SqlRow } from "./project";

const LATVIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearLatviaProjection(db: DatabaseSync): void {
  for (const table of LATVIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeLatviaProjection(
  db: DatabaseSync,
  projection: LatviaProjection,
  attemptId: string,
