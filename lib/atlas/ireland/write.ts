...
import type { IrelandProjection, SqlRow } from "./project";

const IRELAND_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearIrelandProjection(db: DatabaseSync): void {
  for (const table of IRELAND_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeIrelandProjection(
  db: DatabaseSync,
