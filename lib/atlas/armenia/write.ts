...
import type { ArmeniaProjection, SqlRow } from "./project";

const ARMENIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearArmeniaProjection(db: DatabaseSync): void {
  for (const table of ARMENIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeArmeniaProjection(
  db: DatabaseSync,
  projection: ArmeniaProjection,
  attemptId: string,
