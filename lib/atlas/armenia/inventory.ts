...
export type ArmeniaInventory = {
  ...
  tiers: {
    ...
    classifications: Array<{
      ...
      boundary_calendar_review?: unknown;
      [key: string]: unknown;
    }>;
...
function walkRegularFiles(dir: string, relBase: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink() || lstatSync(abs).isSymbolicLink()) {
      throw new Error(`Unexpected symlink in Armenia package: ${rel}`);
    }
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unexpected non-file in Armenia package: ${rel}`);
    }
  }
  return out.sort();
}
...
export class ArmeniaPreflightError extends Error {
...
function extractArmeniaUstar(buffer: Buffer): Map<string, Buffer> {
  ...
  while (offset + 512 <= buffer.length) {
    ...
    if (typeflag === "1" || typeflag === "2") {
      throw new Error(`Armenia payload rejects links: ${full}`);
    }
...
function unpackArmeniaMembers(packageDir: string): Map<string, Buffer> {
  const manifestPath = path.join(packageDir, "manifest.json");
  if (!existsSync(manifestPath)) {
    throw new Error("Armenia manifest.json is missing.");
  }
  ...
  for (const [relative, expected] of Object.entries(manifest.chunks ?? {})) {
    ...
    if (!existsSync(abs)) {
      throw new Error(`Missing Armenia payload chunk ${relative}`);
    }
  ...
  if (adapterSha256(gz) !== manifest.payload_sha256) {
    throw new Error("Armenia concatenated payload checksum mismatch");
  }
  let unzipped: Buffer;
  try {
    unzipped = gunzipSync(gz);
  } catch (error) {
    throw new Error(`Armenia payload is not valid gzip: ${error instanceof Error ? error.message : String(error)}`);
  }
  const files = extractArmeniaUstar(unzipped);
  if (files.size !== EXPECTED_COUNTS.payload_member_files) {
    throw new Error(`Expected ${EXPECTED_COUNTS.payload_member_files} Armenia payload members, found ${files.size}`);
  }
  const inventoryBytes = files.get("inventory.json");
  if (!inventoryBytes) {
    throw new Error("Armenia payload is missing inventory.json");
  }
  ...
  for (const [entry, expected] of Object.entries(contents)) {
    ...
    if (bytes.length !== expected.bytes || adapterSha256(bytes) !== expected.sha256) {
      throw new Error(`Armenia member hash mismatch: ${entry}`);
    }
  }
  for (const entry of files.keys()) {
    if (entry === "inventory.json") continue;
    if (!contents[entry]) {
      throw new Error(`Unexpected Armenia payload member ${entry}`);
    }
  }
  return files;
}

export function scanArmeniaInventory(options: {
  root: string;
  packageDir?: string;
  tierPath?: string;
  ...
  if (sha256Hex(readFileSync(schemaAttempt)) !== ATTEMPT_LOG_SHA256) {
    throw new ArmeniaPreflightError("schema_hash_mismatch", "Attempt-log SQL bytes do not match the Identity Rules digest.", {});
  }
  if (sha256Hex(readFileSync(schemaMaster)) !== MASTER_SCHEMA_SHA256) {
    throw new ArmeniaPreflightError("schema_hash_mismatch", "Master SQL bytes do not match the Identity Rules digest.", {});
  }
  ...
  if (requireGit) {
    ...
    if (tracked.length !== EXPECTED_COUNTS.outer_package_files) {
      throw new ArmeniaPreflightError(
        "package_inventory",
        `Expected ${EXPECTED_COUNTS.outer_package_files} git-tracked Armenia files, found ${tracked.length}.`,
        {},
      );
    }
    if (JSON.stringify(tracked) !== JSON.stringify(walked)) {
      throw new ArmeniaPreflightError(
        "unpinned_worktree",
        "Armenia package worktree files do not match git-tracked paths.",
        {},
      );
    }
  ...
  if (failedFiles.length > 0) {
    throw new ArmeniaPreflightError(
      "package_read",
      `Failed to read Armenia package files: ${failedFiles.map((item) => item.input_path).join(", ")}`,
  ...
  try {
    members = unpackArmeniaMembers(packageDir);
  } catch (error) {
    throw new ArmeniaPreflightError(
      "packed_input",
  ...
  if (tierMeta.error === "missing_tier_file") {
    throw new ArmeniaPreflightError(
      "missing_tier",
      `Approved Armenia tier file is missing at ${TIER_PATH}.`,
      intendedInventory,
    );
  }

  let tierJson: ArmeniaInventory["tiers"];
  try {
    tierJson = JSON.parse(tierBytes!.toString("utf8")) as ArmeniaInventory["tiers"];
  } catch (error) {
    throw new ArmeniaPreflightError(
      "tier_unreadable",
      `Armenia tier file is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      intendedInventory,
    );
  }
  ...
  if (tierJson.status !== "approved") {
    throw new ArmeniaPreflightError(
      "tier_not_approved",
      `Armenia tiers status is ${JSON.stringify(tierJson.status)}; import requires status "approved".`,
      intendedInventory,
    );
  }
  if (tierMeta.sha256 !== TIER_SHA256) {
    throw new ArmeniaPreflightError(
      "tier_hash_mismatch",
      `Approved Armenia tier SHA-256 mismatch; expected ${TIER_SHA256}.`,
      intendedInventory,
    );
  }
  ...
  if (!manifestItem?.text) {
    throw new ArmeniaPreflightError("missing_manifest", "Armenia manifest.json is missing.", intendedInventory);
  }
  const manifest = JSON.parse(manifestItem.text) as Record<string, unknown>;
  if (manifest.country !== "Armenia") {
    throw new ArmeniaPreflightError("manifest_country", "Armenia manifest country mismatch.", intendedInventory);
  }

  const coverageItem = byPath.get(`${PACKAGE_PREFIX}/coverage.json`);
  if (!coverageItem?.text) {
    throw new ArmeniaPreflightError("missing_coverage", "Armenia coverage.json is missing.", intendedInventory);
  }
  const sourceLinksItem = byPath.get(`${UNPACKED_PREFIX}/source-links.json`);
  if (!sourceLinksItem?.text) {
    throw new ArmeniaPreflightError("missing_source_links", "Armenia source-links.json is missing.", intendedInventory);
  }
  ...
  const memberFile = (entry: string) => {
    ...
    if (!item) throw new ArmeniaPreflightError("missing_input", `Missing ${logical}`, intendedInventory);
  ...
  if (!historyIndexItem?.text) {
    throw new ArmeniaPreflightError("missing_input", "Missing history-index.json", intendedInventory);
  }
  const historyIndexRows = JSON.parse(historyIndexItem.text) as Array<Record<string, unknown>>;
  if (!Array.isArray(historyIndexRows)) {
    throw new ArmeniaPreflightError("packed_input", "history-index.json must be an array", intendedInventory);
  }
  ...
  return {
    ...
    sourceLinks: JSON.parse(sourceLinksItem.text) as ArmeniaInventory["sourceLinks"],
