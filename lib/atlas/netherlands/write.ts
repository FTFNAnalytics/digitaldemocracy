...
import type { NetherlandsProjection, SqlRow } from "./project";

const NETHERLANDS_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearNetherlandsProjection(db: DatabaseSync): void {
  for (const table of NETHERLANDS_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeNetherlandsProjection(
  db: DatabaseSync,
