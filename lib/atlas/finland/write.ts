...
import type { FinlandProjection, SqlRow } from "./project";

const FINLAND_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearFinlandProjection(db: DatabaseSync): void {
  for (const table of FINLAND_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeFinlandProjection(
  db: DatabaseSync,
  projection: FinlandProjection,
  attemptId: string,
