...
export type FinlandEvidenceRef = {
  input_path: string;
  sha256?: string;
  locator?: string | null;
...
export type FinlandAuthoredDate = {
  value: string;
  precision: string;
  certainty: string;
  year?: number;
  ...
  day?: number;
  conflicting?: boolean;
  evidence?: FinlandEvidenceRef[];
};

export type FinlandOfficeRow = {
  id_namespace: string;
  office_id: string;
  country_id: string;
  geography_id: string;
  ...
  next_election?: FinlandAuthoredDate | null;
  evidence?: FinlandEvidenceRef[];
  review_notes?: string[];
  notes?: string[];
...
export type FinlandEventRow = {
  ...
  date?: FinlandAuthoredDate | string | null;
...
export type FinlandProceedingRow = {
  id_namespace: string;
  office_id: string;
  history_key: string;
  event_id: string;
...
export type FinlandGeographyRow = {
  geography_id: string;
  name: string;
  country_id: string;
...
export type FinlandResearchGapRow = {
...
export type FinlandCrosswalkRow = {
...
export type FinlandTierClassification = {
  office_id: string;
  tier: string;
...
export type FinlandInventory = {
  ...
  offices: FinlandOfficeRow[];
  events: FinlandEventRow[];
  results: FinlandResultRow[];
  geographies: FinlandGeographyRow[];
  sourceCatalogue: FinlandSourceRow[];
  researchGaps: FinlandResearchGapRow[];
  crosswalks: FinlandCrosswalkRow[];
  proceedings: FinlandProceedingRow[];
  tiers: {
    status: string;
    production_accepted?: boolean;
    classifications: FinlandTierClassification[];
...
function walkRegularFiles(dir: string, relBase: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink() || lstatSync(abs).isSymbolicLink()) {
      throw new Error(`Unexpected symlink in Finland research pack: ${rel}`);
    }
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unexpected non-file in Finland research pack: ${rel}`);
    }
  }
  return out.sort();
}
...
export class FinlandPreflightError extends Error {
...
export function scanFinlandInventory(options: {
  root: string;
  researchDir?: string;
  tierPath?: string;
  ...
  if (sha256Hex(readFileSync(schemaAttempt)) !== ATTEMPT_LOG_SHA256) {
    throw new FinlandPreflightError("schema_hash_mismatch", "Attempt-log SQL bytes do not match the Identity Rules digest.", {});
  }
  if (sha256Hex(readFileSync(schemaMaster)) !== MASTER_SCHEMA_SHA256) {
    throw new FinlandPreflightError("schema_hash_mismatch", "Master SQL bytes do not match the Identity Rules digest.", {});
  }
  ...
  if (requireGit) {
    ...
    if (tracked.length !== EXPECTED_COUNTS.retained_inputs) {
      throw new FinlandPreflightError(
        "package_inventory",
        `Expected ${EXPECTED_COUNTS.retained_inputs} git-tracked Finland research/tier files, found ${tracked.length}.`,
        {},
      );
    }
    if (JSON.stringify(tracked) !== JSON.stringify(walked)) {
      throw new FinlandPreflightError(
        "unpinned_worktree",
        "Finland research worktree files do not match git-tracked paths.",
        {},
      );
    }
  ...
  if (tierMeta.error === "missing_tier_file" || !tierItem) {
    throw new FinlandPreflightError("missing_tier", `Approved Finland tier file is missing at ${TIER_PATH}.`, intendedInventory);
  }

  let tierJson: FinlandInventory["tiers"];
  try {
    tierJson = JSON.parse(tierItem.text ?? "") as FinlandInventory["tiers"];
  } catch (error) {
    throw new FinlandPreflightError(
      "tier_unreadable",
      `Finland tier file is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      intendedInventory,
    );
  }
  ...
  if (tierJson.status !== "approved" || tierJson.production_accepted !== true) {
    throw new FinlandPreflightError(
      "tier_not_approved",
      `Finland tiers status is ${JSON.stringify(tierJson.status)}; import requires approved production-accepted bytes.`,
      intendedInventory,
    );
  }
  if (tierItem.sha256 !== TIER_SHA256) {
    throw new FinlandPreflightError(
      "tier_hash_mismatch",
      `Finland tier SHA-256 mismatch; expected ${TIER_SHA256}.`,
      intendedInventory,
    );
  }
  ...
  if (tracked.length !== EXPECTED_COUNTS.retained_inputs) {
    throw new FinlandPreflightError(
      "package_inventory",
      `Expected ${EXPECTED_COUNTS.retained_inputs} retained Finland inputs, found ${tracked.length}.`,
      intendedInventory,
    );
  }
  ...
  for (const rel of mustRead) {
    if (!byPath.has(rel)) {
      throw new FinlandPreflightError("missing_research_table", `Missing required Finland research table ${rel}.`, intendedInventory);
    }
  }
  ...
  return {
    ...
    hashInputs,
    offices: readJsonArray<FinlandOfficeRow>(byPath.get(REGISTER_RELATIVE)!.absPath, REGISTER_RELATIVE),
    events: readJsonArray<FinlandEventRow>(byPath.get(EVENTS_RELATIVE)!.absPath, EVENTS_RELATIVE),
    results: readJsonArray<FinlandResultRow>(byPath.get(RESULTS_RELATIVE)!.absPath, RESULTS_RELATIVE),
    geographies: readJsonArray<FinlandGeographyRow>(byPath.get(GEOGRAPHY_RELATIVE)!.absPath, GEOGRAPHY_RELATIVE),
    sourceCatalogue: readJsonArray<FinlandSourceRow>(byPath.get(SOURCES_RELATIVE)!.absPath, SOURCES_RELATIVE),
    researchGaps: readJsonArray<FinlandResearchGapRow>(byPath.get(GAPS_RELATIVE)!.absPath, GAPS_RELATIVE),
    crosswalks: readJsonArray<FinlandCrosswalkRow>(byPath.get(CROSSWALK_RELATIVE)!.absPath, CROSSWALK_RELATIVE),
    proceedings: readJsonArray<FinlandProceedingRow>(byPath.get(PROCEEDINGS_RELATIVE)!.absPath, PROCEEDINGS_RELATIVE),
    tiers: tierJson,
    intendedInventory,
  };
}
