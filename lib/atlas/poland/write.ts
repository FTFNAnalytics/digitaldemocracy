...
import type { PolandProjection, SqlRow } from "./project";

const POLAND_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
/**
 * Tables written by one Poland publication. Checked after the bulk insert.
 * research_date has deferred self-FKs on range endpoints. Node's SQLite still
 * records those constraints per row, and 22k Poland dates make that check
 * quadratic (about 14s locally, long enough to trip Vitest's 60s worker RPC
 * timeout). Foreign keys are suspended for this transaction and verified once
 * with foreign_key_check before commit.
 */
const POLAND_FK_TABLES = [
  "dataset_lineage",
  "dataset_release",
  "publication_release",
  "retained_input",
...
export function clearPolandProjection(db: DatabaseSync): void {
  for (const table of POLAND_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writePolandProjection(
  db: DatabaseSync,
  projection: PolandProjection,
  attemptId: string,
...
function assertForeignKeys(db: DatabaseSync): void {
  for (const table of POLAND_FK_TABLES) {
