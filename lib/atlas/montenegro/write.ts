...
import type { MontenegroProjection, SqlRow } from "./project";

const MONTENEGRO_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearMontenegroProjection(db: DatabaseSync): void {
  for (const table of MONTENEGRO_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeMontenegroProjection(
  db: DatabaseSync,
  projection: MontenegroProjection,
  attemptId: string,
