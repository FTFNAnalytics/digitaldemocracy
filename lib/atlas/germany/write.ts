...
import type { GermanyProjection, SqlRow } from "./project";

const GERMANY_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearGermanyProjection(db: DatabaseSync): void {
  for (const table of GERMANY_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeGermanyProjection(
  db: DatabaseSync,
