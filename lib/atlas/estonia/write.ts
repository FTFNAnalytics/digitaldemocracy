...
import type { EstoniaProjection, SqlRow } from "./project";

const ESTONIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearEstoniaProjection(db: DatabaseSync): void {
  for (const table of ESTONIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeEstoniaProjection(
  db: DatabaseSync,
  projection: EstoniaProjection,
  attemptId: string,
