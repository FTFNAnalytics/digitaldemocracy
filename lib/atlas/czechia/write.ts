...
import type { CzechiaProjection, SqlRow } from "./project";

const CZECHIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
/**
 * Tables written by one Czechia publication. Checked after the bulk insert.
 * research_date has deferred self-FKs on range endpoints. Node's SQLite still
 * records those constraints per row, and ~52k Czechia dates make that check
...
export function clearCzechiaProjection(db: DatabaseSync): void {
  for (const table of CZECHIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeCzechiaProjection(
  db: DatabaseSync,
  projection: CzechiaProjection,
  attemptId: string,
...
function assertForeignKeys(db: DatabaseSync): void {
  for (const table of CZECHIA_FK_TABLES) {
