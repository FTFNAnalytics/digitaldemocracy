...
import type { NorthMacedoniaProjection, SqlRow } from "./project";

const NORTH_MACEDONIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearNorthMacedoniaProjection(db: DatabaseSync): void {
  for (const table of NORTH_MACEDONIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeNorthMacedoniaProjection(
  db: DatabaseSync,
  projection: NorthMacedoniaProjection,
  attemptId: string,
  ...
  try {
    ...
    if (!selected) {
      ...
      clearNorthMacedoniaProjection(db);
...
function upsertReceipt(db: DatabaseSync, attemptId: string, releaseId: string): void {
  ...
  db.prepare(
    "UPDATE publication_receipt SET last_publish_attempt_id = ?, attempted_lineage_id = ?, attempted_release_id = ? WHERE singleton = 1",
