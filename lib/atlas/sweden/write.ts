...
import type { SwedenProjection, SqlRow } from "./project";

const SWEDEN_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearSwedenProjection(db: DatabaseSync): void {
  for (const table of SWEDEN_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeSwedenProjection(
  db: DatabaseSync,
