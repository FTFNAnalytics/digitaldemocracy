...
import type { MaltaProjection, SqlRow } from "./project";

const MALTA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearMaltaProjection(db: DatabaseSync): void {
  for (const table of MALTA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeMaltaProjection(
  db: DatabaseSync,
  projection: MaltaProjection,
  attemptId: string,
