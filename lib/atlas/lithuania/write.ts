...
import type { LithuaniaProjection, SqlRow } from "./project";

const LITHUANIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearLithuaniaProjection(db: DatabaseSync): void {
  for (const table of LITHUANIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeLithuaniaProjection(
  db: DatabaseSync,
  projection: LithuaniaProjection,
  attemptId: string,
