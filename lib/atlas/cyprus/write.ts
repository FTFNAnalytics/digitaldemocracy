...
import type { CyprusProjection, SqlRow } from "./project";

const CYPRUS_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearCyprusProjection(db: DatabaseSync): void {
  for (const table of CYPRUS_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeCyprusProjection(
  db: DatabaseSync,
  projection: CyprusProjection,
  attemptId: string,
