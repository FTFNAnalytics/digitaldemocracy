...
import type { GreeceProjection, SqlRow } from "./project";

const GREECE_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearGreeceProjection(db: DatabaseSync): void {
  for (const table of GREECE_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeGreeceProjection(
  db: DatabaseSync,
  projection: GreeceProjection,
  attemptId: string,
