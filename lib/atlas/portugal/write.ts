...
import type { PortugalProjection, SqlRow } from "./project";

const PORTUGAL_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearPortugalProjection(db: DatabaseSync): void {
  for (const table of PORTUGAL_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writePortugalProjection(
  db: DatabaseSync,
  projection: PortugalProjection,
  attemptId: string,
  options?: { reuseRelease?: boolean },
): void {
  // Portugal adds ~100k offices/events/results plus ~200k evidence rows onto the
  // already-published master. The default 2MB page cache turns that into a
  // CPU-bound rewrite of the whole file.
