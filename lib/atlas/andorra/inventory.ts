...
function walkRegularFiles(dir: string, relBase: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink() || lstatSync(abs).isSymbolicLink()) {
      throw new Error(`Unexpected symlink in Andorra package: ${rel}`);
    }
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unexpected non-file in Andorra package: ${rel}`);
    }
  }
  return out.sort();
}
...
export class AndorraPreflightError extends Error {
...
export function scanAndorraInventory(options: {
  root: string;
  packageDir?: string;
  tierPath?: string;
  ...
  if (sha256Hex(readFileSync(schemaAttempt)) !== ATTEMPT_LOG_SHA256) {
    throw new AndorraPreflightError("schema_hash_mismatch", "Attempt-log SQL bytes do not match the Identity Rules digest.", {});
  }
  if (sha256Hex(readFileSync(schemaMaster)) !== MASTER_SCHEMA_SHA256) {
    throw new AndorraPreflightError("schema_hash_mismatch", "Master SQL bytes do not match the Identity Rules digest.", {});
  }
  ...
  if (requireGit) {
    ...
    if (tracked.length !== EXPECTED_COUNTS.package_files_retained) {
      throw new AndorraPreflightError(
        "package_inventory",
        `Expected ${EXPECTED_COUNTS.package_files_retained} git-tracked Andorra files, found ${tracked.length}.`,
        {},
      );
    }
    if (JSON.stringify(tracked) !== JSON.stringify(walked)) {
      throw new AndorraPreflightError(
        "unpinned_worktree",
        "Andorra package worktree files do not match git-tracked paths.",
        {},
      );
    }
  ...
  if (failedFiles.length > 0) {
    throw new AndorraPreflightError(
      "package_read",
      `Failed to read Andorra package files: ${failedFiles.map((item) => item.input_path).join(", ")}`,
      buildIntendedInventory({ gitCommit, packageFiles: packageFilesMeta, tier: { input_path: TIER_PATH, sha256: null, byte_count: null, status: null } }),
    );
  }
  ...
  if (tierMeta.error === "missing_tier_file") {
    throw new AndorraPreflightError(
      "missing_tier",
      `Approved Andorra tier file is missing at ${TIER_PATH}.`,
      intendedInventory,
    );
  }

  let tierJson: AndorraInventory["tiers"];
  try {
    tierJson = JSON.parse(tierBytes!.toString("utf8")) as AndorraInventory["tiers"];
  } catch (error) {
    throw new AndorraPreflightError(
      "tier_unreadable",
      `Andorra tier file is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      intendedInventory,
    );
  }
  ...
  if (tierJson.status !== "approved") {
    throw new AndorraPreflightError(
      "tier_not_approved",
      `Andorra tiers status is ${JSON.stringify(tierJson.status)}; import requires status "approved".`,
      intendedInventory,
    );
  }
  if (tierMeta.sha256 !== TIER_SHA256) {
    throw new AndorraPreflightError(
      "tier_hash_mismatch",
      `Approved Andorra tier SHA-256 mismatch; expected ${TIER_SHA256}.`,
      intendedInventory,
    );
  }
  ...
  if (!manifestPath?.text) {
    throw new AndorraPreflightError("missing_manifest", "Andorra manifest.json is missing.", intendedInventory);
  }
  ...
  if (manifest.country !== "Andorra" || manifest.country_code !== "AD") {
    throw new AndorraPreflightError("manifest_country", "Andorra manifest country/code mismatch.", intendedInventory);
  }
  if (manifest.coverage_complete !== false) {
    throw new AndorraPreflightError("coverage_complete", "Frozen Andorra coverage_complete must be false.", intendedInventory);
  }
  ...
  const pkgFile = (rel: string) => {
    ...
    if (!item) throw new AndorraPreflightError("missing_input", `Missing ${logical}`, intendedInventory);
  ...
  return {
    ...
    sourceLinks: JSON.parse(byPath.get(`${PACKAGE_PREFIX}/source-links.json`)!.text!) as AndorraInventory["sourceLinks"],
