...
import type { AndorraProjection, SqlRow } from "./project";

const ANDORRA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearAndorraProjection(db: DatabaseSync): void {
  for (const table of ANDORRA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeAndorraProjection(
  db: DatabaseSync,
  projection: AndorraProjection,
  attemptId: string,
