...
import type { SqlRow, UnitedKingdomProjection } from "./project";

const UNITED_KINGDOM_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearUnitedKingdomProjection(db: DatabaseSync): void {
  for (const table of UNITED_KINGDOM_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeUnitedKingdomProjection(
  db: DatabaseSync,
  projection: UnitedKingdomProjection,
  attemptId: string,
  ...
  try {
    ...
    if (!selected) {
      ...
      clearUnitedKingdomProjection(db);
