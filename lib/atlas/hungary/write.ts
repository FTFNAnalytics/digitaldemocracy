...
import type { HungaryProjection, SqlRow } from "./project";

const HUNGARY_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearHungaryProjection(db: DatabaseSync): void {
  for (const table of HUNGARY_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeHungaryProjection(
  db: DatabaseSync,
...
function assertForeignKeys(db: DatabaseSync): void {
  for (const table of HUNGARY_FK_TABLES) {
