...
import type { SqlRow, SwitzerlandProjection } from "./project";

const SWITZERLAND_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearSwitzerlandProjection(db: DatabaseSync): void {
  for (const table of SWITZERLAND_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeSwitzerlandProjection(
  db: DatabaseSync,
  projection: SwitzerlandProjection,
  attemptId: string,
