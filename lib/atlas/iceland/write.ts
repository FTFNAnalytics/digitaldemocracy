...
import type { IcelandProjection, SqlRow } from "./project";

const ICELAND_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearIcelandProjection(db: DatabaseSync): void {
  for (const table of ICELAND_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeIcelandProjection(
  db: DatabaseSync,
  projection: IcelandProjection,
  attemptId: string,
