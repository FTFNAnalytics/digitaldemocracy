...
import type { SerbiaInventory, SerbiaRegisterOffice, SerbiaTierOffice } from "./inventory";

export type SqlRow = Record<string, unknown>;

export type SerbiaProjection = {
  lineage: SqlRow;
  release: SqlRow;
...
export function projectSerbia(inventory: SerbiaInventory): SerbiaProjection {
  const L = LINEAGE_ID;
  ...
  const country: SqlRow = {
    ...
    raw_json: rawEnvelope({
      ...
      supplemental: {
        ...
        successor_edges: inventory.statusChanges.length,
        successor_edges_are_status_changes: true,
        research_coverage_complete: false,
      },
    }),
  };

  const paired: Array<{ tier: SerbiaTierOffice; register: SerbiaRegisterOffice }> = [];
  ...
  const geographies: SqlRow[] = inventory.geographies.map((geography) => ({
    ...
    raw_json: rawEnvelope({
      ...
      row: geography,
      supplemental: { supplied_geography_name: geography.supplied, successor_edge: null },
    }),
  }));
  ...
  const offices: SqlRow[] = paired.map(({ tier, register }) => ({
    ...
    raw_json: rawEnvelope({
      ...
      supplemental: {
        ...
        geography_name_supplied: register.geography_name_supplied,
        parent_geography_not_an_edge: register.parent_geography,
  ...
  const crosswalks: SqlRow[] = inventory.statusChanges.map((edge) => {
    const successorKey = officeRecordKey.get(edge.successor_office_id);
    if (!successorKey) throw new Error(`Missing successor office ${edge.successor_office_id}`);
    return {
      entity_kind: "office",
      upstream_namespace: STATUS_CHANGE_NAMESPACE,
      upstream_id: edge.predecessor_office_id,
      ...
      raw_json: rawEnvelope({
        ...
        row: edge,
        supplemental: { guessed_merger: false, boundary_change_claim: false },
      }),
    };
  });
  ...
  const unresolved: SqlRow[] = inventory.gaps.map((gap, index) => {
    ...
    return {
      unresolved_id: serbiaUnresolvedId(countryRec, occurrence, gap.gap_id),
      record_key: countryRec,
      ...
      raw_json: rawEnvelope({
        ...
        row: { token: gap.gap_id, status: gap.status, topic: gap.topic, closed: false },
      }),
    };
  });

  const countType = (officeType: string, status?: SerbiaRegisterOffice["office_status"]) =>
  ...
  const validatedCounts: Record<string, number> = {
    ...
    direct_local_executive_offices: paired.filter((row) => row.register.direct_executive && row.register.scope === "local").length,
    direct_provincial_executive_offices: paired.filter((row) => row.register.direct_executive && row.register.scope === "provincial").length,
    current_local_assemblies: countType("local_assembly", "current"),
    city_municipality_assemblies: countType("city_municipality_assembly", "current"),
    current_councils_chambers_assemblies: paired.filter(
      (row) => row.register.office_status === "current" && row.register.office_type !== "president",
    ).length,
    current_national: paired.filter((row) => row.register.scope === "national").length,
    provincial_offices: countType("provincial_assembly"),
    ep_offices: 0,
    kosovo_scope_offices: 0,
    evidence_links: 0,
  };
  for (const [countKey, expected] of Object.entries(EXPECTED_COUNTS)) {
    if (validatedCounts[countKey] !== expected) throw new Error(`Serbia ${countKey} count ${validatedCounts[countKey]} != ${expected}`);
  }
  if ("documented_result_rows_omitted" in validatedCounts || "documented_event_rows_omitted" in validatedCounts || "documented_sources_omitted" in validatedCounts) {
  ...
  return {
    lineage: {
      ...
      description:
        "Serbia Prompt AX rebuilt register. Justin accepted 173 current and 5 historical-only offices with RS-AX-G01 through RS-AX-G11 left open. 0 published events, 0 published result rows, and 0 published sources. Five same-territory status-change edges only. Kosovo-scope offices stay 0.",
    },
    release: {
      lineage_id: L,
      release_id: R,
