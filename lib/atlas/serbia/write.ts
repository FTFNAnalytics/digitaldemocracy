...
import type { SerbiaProjection, SqlRow } from "./project";

const SERBIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearSerbiaProjection(db: DatabaseSync): void {
  for (const table of SERBIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeSerbiaProjection(
  db: DatabaseSync,
  projection: SerbiaProjection,
  attemptId: string,
