...
import type { LuxembourgProjection, SqlRow } from "./project";

const LUXEMBOURG_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearLuxembourgProjection(db: DatabaseSync): void {
  for (const table of LUXEMBOURG_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeLuxembourgProjection(
  db: DatabaseSync,
  projection: LuxembourgProjection,
  attemptId: string,
