...
const ALBANIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearAlbaniaProjection(db: DatabaseSync): void {
  for (const table of ALBANIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeAlbaniaProjection(
  db: DatabaseSync,
  projection: AlbaniaProjection,
  attemptId: string,
