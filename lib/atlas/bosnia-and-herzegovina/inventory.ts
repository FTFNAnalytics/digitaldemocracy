...
function walkRegularFiles(dir: string, relBase: string): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink() || lstatSync(abs).isSymbolicLink()) {
      throw new Error(`Unexpected symlink in Bosnia and Herzegovina package: ${rel}`);
    }
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unexpected non-file in Bosnia and Herzegovina package: ${rel}`);
    }
  }
  return out.sort();
}
...
function extractBosniaUstar(buffer: Buffer): Map<string, Buffer> {
  ...
  while (offset + 512 <= buffer.length) {
    ...
    if (typeflag === "1" || typeflag === "2") {
      throw new Error(`Bosnia and Herzegovina payload rejects links: ${full}`);
    }
...
export function unpackBosniaMembers(packageDir: string): Map<string, Buffer> {
  const manifestPath = path.join(packageDir, "manifest.json");
  if (!existsSync(manifestPath)) {
    throw new Error("Bosnia and Herzegovina manifest.json is missing.");
  }
  ...
  for (const [relative, expected] of Object.entries(manifest.chunks ?? {})) {
    ...
    if (!existsSync(abs)) {
      throw new Error(`Missing Bosnia and Herzegovina payload chunk ${relative}`);
    }
    const bytes = readFileSync(abs);
    if (bytes.length !== expected.bytes || adapterSha256(bytes) !== expected.sha256) {
      throw new Error(`Bosnia and Herzegovina payload chunk mismatch: ${relative}`);
    }
    parts.push(bytes);
  }
  const gz = Buffer.concat(parts);
  if (adapterSha256(gz) !== manifest.payload_sha256) {
    throw new Error("Bosnia and Herzegovina concatenated payload checksum mismatch");
  }
  let unzipped: Buffer;
  try {
    ...
    throw new Error(
      `Bosnia and Herzegovina payload is not valid gzip: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  const files = extractBosniaUstar(unzipped);
  if (files.size !== EXPECTED_COUNTS.payload_member_files) {
    throw new Error(`Expected ${EXPECTED_COUNTS.payload_member_files} Bosnia payload members, found ${files.size}`);
  }
  const inventoryBytes = files.get("inventory.json");
  if (!inventoryBytes) {
    throw new Error("Bosnia and Herzegovina payload is missing inventory.json");
  }
  ...
  for (const [entry, expected] of Object.entries(contents)) {
    ...
    if (bytes.length !== expected.bytes || adapterSha256(bytes) !== expected.sha256) {
      throw new Error(`Bosnia and Herzegovina member hash mismatch: ${entry}`);
    }
  }
  for (const entry of files.keys()) {
    if (entry === "inventory.json") continue;
    if (!contents[entry]) {
      throw new Error(`Unexpected Bosnia and Herzegovina payload member ${entry}`);
    }
  }
  return files;
}
...
export function scanBosniaInventory(options: {
  ...
  if (requireGit) {
    ...
    if (tracked.length !== EXPECTED_COUNTS.outer_package_files) {
      throw new BosniaPreflightError(
        "package_inventory",
        `Expected ${EXPECTED_COUNTS.outer_package_files} git-tracked Bosnia and Herzegovina files, found ${tracked.length}.`,
        {},
      );
    }
    if (JSON.stringify(tracked) !== JSON.stringify(walked)) {
      throw new BosniaPreflightError(
        "unpinned_worktree",
        "Bosnia and Herzegovina package worktree files do not match git-tracked paths.",
        {},
      );
    }
  ...
  if (failedFiles.length > 0) {
    throw new BosniaPreflightError(
      "package_read",
      `Failed to read Bosnia and Herzegovina package files: ${failedFiles.map((item) => item.input_path).join(", ")}`,
  ...
  try {
    members = unpackBosniaMembers(packageDir);
  } catch (error) {
    throw new BosniaPreflightError(
      "packed_input",
  ...
  if (tierMeta.error === "missing_tier_file") {
    throw new BosniaPreflightError(
      "missing_tier",
      `Approved Bosnia and Herzegovina tier file is missing at ${TIER_PATH}.`,
      intendedInventory,
    );
  }
  ...
  try {
    ...
    throw new BosniaPreflightError(
      "tier_unreadable",
      `Bosnia and Herzegovina tier file is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      intendedInventory,
    );
  }
  ...
  if (tierJson.status !== "approved") {
    throw new BosniaPreflightError(
      "tier_not_approved",
      `Bosnia and Herzegovina tiers status is ${JSON.stringify(tierJson.status)}; import requires status "approved".`,
      intendedInventory,
    );
  }
  ...
  if (!manifestItem?.text) {
    throw new BosniaPreflightError("missing_manifest", "Bosnia and Herzegovina manifest.json is missing.", intendedInventory);
  }
  const manifest = JSON.parse(manifestItem.text) as Record<string, unknown>;
  if (manifest.country !== "Bosnia and Herzegovina") {
    throw new BosniaPreflightError("manifest_country", "Bosnia and Herzegovina manifest country mismatch.", intendedInventory);
  }

  const coverageItem = byPath.get(`${PACKAGE_PREFIX}/coverage.json`);
  if (!coverageItem?.text) {
    throw new BosniaPreflightError("missing_coverage", "Bosnia and Herzegovina coverage.json is missing.", intendedInventory);
  }
  const sourceLinksItem = byPath.get(`${UNPACKED_PREFIX}/source-links.json`);
  if (!sourceLinksItem?.text) {
    throw new BosniaPreflightError("missing_source_links", "Bosnia and Herzegovina source-links.json is missing.", intendedInventory);
  }
  ...
  const memberFile = (entry: string) => {
    ...
    if (!item) throw new BosniaPreflightError("missing_input", `Missing ${logical}`, intendedInventory);
  ...
  if (!historyIndexItem?.text) {
    throw new BosniaPreflightError("missing_input", "Missing history-index.json", intendedInventory);
  }
  const historyIndexRows = JSON.parse(historyIndexItem.text) as Array<Record<string, unknown>>;
  if (!Array.isArray(historyIndexRows)) {
    throw new BosniaPreflightError("packed_input", "history-index.json must be an array", intendedInventory);
  }
  ...
  return {
    ...
    sourceLinks: JSON.parse(sourceLinksItem.text) as BosniaInventory["sourceLinks"],
    officeRegister: memberFile("tables/master/office-register.json"),
    histories: memberFile("tables/master/history-index.json"),
    historyIndex: {
      relativePath: `${UNPACKED_PREFIX}/history-index.json`,
      archiveEntry: "history-index.json",
      rows: historyIndexRows,
    },
    returns: memberFile("tables/master/detailed-returns.json"),
