...
import type { BosniaProjection, SqlRow } from "./project";

const BOSNIA_PROJECTION_TABLES = [
  "evidence_link",
  "unresolved_evidence",
  "identity_crosswalk",
  "record_locator",
...
export function clearBosniaProjection(db: DatabaseSync): void {
  for (const table of BOSNIA_PROJECTION_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE lineage_id = ?`).run(LINEAGE_ID);
  }
}

export function writeBosniaProjection(
  db: DatabaseSync,
  projection: BosniaProjection,
  attemptId: string,
