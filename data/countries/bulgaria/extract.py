...
COUNTRY = 'Bulgaria'
CODE = 'BG'
COMPANION = 'Bulgaria_Subnational_Elections_Updated.xlsx'
...
def main(archive_path):
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for sheet, number, row, formulas in workbook_rows(source(BOOK)):
            ...
            elif row.get("Country") == COUNTRY or row.get("Country or territory") == COUNTRY or (CODE is not None and str(row.get("Office ID", "")).startswith(CODE + "-")) or sheet in ("Parameters", "Read me"):
        ...
        name = "Office_Briefings/" + 'bulgaria' + ".html"
        ...
        companion_ids = set()
        if COMPANION:
            table=tables.get("companion/jurisdictions", tables.get("companion/office-register"))
            companion_ids={r[table["columns"].index("Jurisdiction ID" if "Jurisdiction ID" in table["columns"] else "Office ID")] for r in table["rows"]}
            assert companion_ids <= ids
            ch=tables.get("companion/histories", tables.get("companion/history-index"))
            assert len(ch["rows"]) == sum(r["Office ID"] in companion_ids for r in history)
        summary={"country":COUNTRY,"office_records":len(ids),"history_entries":len(history),"calendar_cohorts":len(tables.get("master/election-calendar",{}).get("rows",[])),"companion_offices":len(companion_ids),"offices_outside_companion":sorted(ids-companion_ids),"companion_full_result_rows":len(tables.get("companion/full-results",tables.get("companion/detailed-returns",{})).get("rows",[])),"office_briefings":len(ids),"table_row_counts":{k:len(v["rows"]) for k,v in tables.items()}}
        ...
        compressed=lzma.compress(payload.getvalue(),filters=[{"id":lzma.FILTER_LZMA2,"preset":9,"dict_size":256*1024*1024}])
        chunks={}
        (ROOT/"payload").mkdir(exist_ok=True)
        for n,start in enumerate(range(0,len(compressed),98304),1):
            name=f"payload/data.tar.xz.part{n:03d}"
            raw=compressed[start:start+98304]
            (ROOT/name).write_bytes(raw)
            chunks[name]={"sha256":digest(raw),"bytes":len(raw)}
        manifest={"country":COUNTRY,"research_snapshot":"2026-09-11","packaged":"2026-09-15","window":{"start":"2026-09-08","end":"2028-03-08","inclusive":True},"website_ingestion":"pending","archive_name":Path(archive_path).name,"archive_sha256":digest(Path(archive_path).read_bytes()),"source_entries":inputs,"summary":summary,"compression":"Concatenate chunks in manifest order to obtain a XZ-compressed POSIX tar archive.","payload_sha256":digest(compressed),"chunks":chunks,"contents":{n:{"sha256":digest(b),"bytes":len(b)} for n,b in sorted(contents.items())},"transformations":["Original HTML bytes preserved unchanged. Companion workbook populated-sheet values and formula caches extracted; original XLSX remains in the regional ZIP, with its source checksum retained.","Table arrays preserve row order, headers, original source row numbers, numeric precision and nulls. Recognized Excel dates converted using 1900 epoch to ISO dates; partial date strings remain unchanged.","Formula text, attributes and original caches retained separately; formulas are not recalculated.","History index is filtered from the regional CSV and overlaps master/companion history tables where present. Do not sum these datasets. First-round, unresolved-history and ongoing-election rows remain separate from completed histories.","Master Parameters and Read me are regional methodology, not country totals."]}
