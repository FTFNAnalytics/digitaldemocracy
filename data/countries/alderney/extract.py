"""Extract Alderney from the preserved Europe ZIP; no third-party packages.

Usage: python data/countries/alderney/extract.py /path/to/Europe_...zip
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for sheet, number, row, formulas in workbook_rows(source(BOOK)):
            if sheet == "Sources":
                ...
                continue
            selected = row.get("Country") == "Alderney" or row.get("Country or territory") == "Alderney" or str(row.get("Office ID", "")).startswith("GG-ALD-")
        ...
        assert len(office_ids) == len(office_rows) == 2
        links = set()
        ...
        coverage = next(r for r in coverage_rows if r["country"] == "Alderney")
        ...
        history = [r for r in history if r["Country"] == "Alderney"]
        ...
        index = "<!doctype html><html lang=\"en\"><meta charset=\"utf-8\"><title>Alderney research briefings</title><h1>Alderney research briefings</h1><p>Research checked through 11 September 2026. Partial coverage; final 2026 calendar approval and primary numerical comparisons remain unverified. <a href=\"README.md\">Read scope and limitations</a>.</p><ul>"
        ...
        write("alderney.html", (index + "</ul></html>\n").encode())
        counts = {sheet: len(rows) for sheet, rows in tables.items()}
        assert counts["History index"] == len(history) == 6
        manifest = {"schema_version": "europe-country-extract/1", "country": "Alderney", "country_code": "GG-ALD", "region": "Europe", "research_checked_through": "2026-09-11", "packaged_on": "2026-09-15", "window": {"start": "2026-09-08", "end": "2028-03-08", "inclusive": True}, "coverage_complete": False, "site_ingestion_status": "pending_adapter", "archive": {"filename": Path(archive_path).name, "bytes": Path(archive_path).stat().st_size, "sha256": digest(Path(archive_path).read_bytes())}, "source_inputs": inputs, "table_row_counts": counts, "individual_briefings": len(office_ids), "source_link_counts": {"registered": len(registered), "inline_only": len(links - registered)}, "files": files}
