...
import { dateCertaintyLabel, formatResearchDate } from "@/lib/observatory/dates";
import { eventKindLabel, formatNumeric, formatShare, legalOutcomeLabel } from "@/lib/observatory/format";
import {
  getCountry,
  getEvent,
  getEvents,
...
export function generateStaticParams() {
  return getEvents().map((event) => ({ eventId: event.id }));
}
  ...
  return (
    <article>
      ...
      <dl className="mb-8 grid gap-3 sm:grid-cols-2">
        ...
        <Fact label="Share unit" value={event.voteShareUnit.replaceAll("_", " ")} />
          ...
          <div className="mt-4 grid gap-6 lg:grid-cols-2">
            <ShareBars
              rows={event.resultRows.map((row) => ({
                label: row.label,
                percent: row.share.value == null ? null : toPercent(row.share.value, row.shareUnit),
                note: row.share.status,
              }))}
            />
            <DataTable
              caption={`Full results for ${event.id}`}
              columns={["Label", "Party", "Votes", "Share", "Seats", "Elected", "Substitute", "Status"]}
              rows={event.resultRows.map((row) => [
                row.label,
                ...
                row.electedFlag == null ? "Unknown" : row.electedFlag ? "Yes" : "No",
      ...
      <CiteBlock title={`${office?.names.official ?? event.officeId}, ${formatResearchDate(event.date)}`} path={obsRoutes.event(event.id)} />
    </article>
  );
}
