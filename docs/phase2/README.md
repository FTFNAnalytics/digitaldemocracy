...
1. **Prompt D maps are the import contract; approved packs now load.** `npm run import:atlas` (default `ATLAS_IMPORT_SCOPE=all`) imports Albania, Andorra, Alderney, Armenia, Belgium, Bosnia and Herzegovina, Bulgaria, Denmark, plus Batch A+B packs. Residual-heavy drafts are skipped. See [Continuity_Import.md](Continuity_Import.md).
