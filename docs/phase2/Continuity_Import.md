...
| `romania` | Prompt AL slim pack only (**6,460 current + 0 historical**; 6,372 municipal / 84 regional / 3 national / 1 other; every classification stays `needs_review`; 19,343 events; 23 national/EP result rows). Not part of `all` |
| `greece` | Prompt AM slim pack only (**693 current + 10 historical**; 674 municipal / 26 regional / 2 national / 1 other; every classification stays `needs_review`; 2,774 events; 3,555 proceedings; 14,004 result rows). Not part of `all` |
| `luxembourg` | Prompt AO slim pack only (**102 current + 28 historical**; 128 municipal / 0 regional / 1 national / 1 other; every classification stays `needs_review`; 438 events; 0 result rows; 48,197 omitted). Not part of `all` |
| `malta` | Prompt AP slim pack only (**213 current + 2 historical**; 204 municipal / 8 regional / 2 national / 1 other; every classification stays `needs_review`; 223 events; 0 result rows; 4,084 result rows and 64,204 STV observations omitted). Not part of `all` |
| `cyprus` | Prompt AQ slim pack only (**714 current + 174 historical**; 877 municipal / 5 regional / 5 national / 1 other; every classification stays `needs_review`; 1,599 events; 0 result rows; 11,112 omitted). Not part of `all` |
| `france` | Prompt AR slim pack only (**35,112 current + 2,738 historical**; 37,705 municipal / 141 regional / 4 national / 0 other; every classification stays `needs_review`; 0 events; 0 result rows; 119,554 events and 1,193,657 results omitted). Not part of `all` |
| `germany` | Prompt AS slim pack only (**21,960 current + 670 historical**; numeric tiers 3 / 20 / 552 / 22,055; every classification stays `needs_review`; 0 events; 0 result rows; omitted results and events are not given omitted-total counters). Not part of `all` |
| `united_kingdom` | Prompt AU slim pack only (**482 current + 2 shadow + 26 historical-only**; numeric tiers 2 / 5 / 76 / 427; every classification stays `needs_review`; 0 events; 0 result rows; omitted results and events are not given omitted-total counters). Not part of `all` |
| `italy` | Prompt AT slim pack only (**15,917 current + 696 historical + 8 pending FVG**; numeric tiers 4 / 38 / 11 / 16,568; every classification stays `needs_review`; 0 events; 0 result rows; omitted results and events are not given omitted-total counters). Not part of `all` |
...
- Romania is Prompt AL (`country-package-romania`; **6,460 current + 0 historical**). 6,372 municipal / 84 regional / 3 national / 1 other. Every classification stays `needs_review`. Holds RO-G01–RO-G07 stay open. 19,343 events and 23 national/EP result rows. Local result vectors stay absent. Run with `ATLAS_IMPORT_SCOPE=romania` only. `all` does not import Romania. See [Romania_Import.md](../phase1/romania/Romania_Import.md).
- Greece is Prompt AM (`country-package-greece`; **693 current + 10 historical**). 674 municipal / 26 regional / 2 national / 1 other. Every classification stays `needs_review`. Holds GR-G01–GR-G10 and 14 local source holds stay open. 2,774 events, 3,555 proceedings, and 14,004 result rows from 8,021 observations. Slim land omits `sources/`; source rows are not invented. Run with `ATLAS_IMPORT_SCOPE=greece` only. `all` does not import Greece. See [Greece_Import.md](../phase1/greece/Greece_Import.md).
- Luxembourg is Prompt AO (`country-package-luxembourg`; **102 current + 28 historical**). 128 municipal / 0 regional / 1 national / 1 other. Every classification stays `needs_review`. Holds LU-G03–LU-G11 stay open. LU-G01 and LU-G02 stay resolved exclusions. 438 events and 0 published result rows. Slim land omits `results.json` (48,197 rows) and `sources/`; those rows are not invented. 28 explicit predecessor edges only. Run with `ATLAS_IMPORT_SCOPE=luxembourg` only. `all` does not import Luxembourg. See [Luxembourg_Import.md](../phase1/luxembourg/Luxembourg_Import.md).
- Malta is Prompt AP (`country-package-malta`; **213 current + 2 historical**). 204 municipal / 8 regional / 2 national / 1 other. Every classification stays `needs_review`. Named holds stay open. 223 events and 0 published result rows. Slim land omits `results.json` (4,084 rows), `stv-counts.json` (64,204 observations), and `sources/`; those rows are not invented. The successor crosswalk stays empty. Run with `ATLAS_IMPORT_SCOPE=malta` only. `all` does not import Malta. See [Malta_Import.md](../phase1/malta/Malta_Import.md).
- Cyprus is Prompt AQ (`country-package-cyprus`; **714 current + 174 historical**). 877 municipal / 5 regional / 5 national / 1 other. Every classification stays `needs_review`. Holds CY-G01–CY-G15 stay open. 1,599 events and 0 published result rows. Slim land omits `results.json` (11,112 rows) and `sources/`; those rows are not invented. The successor crosswalk stays empty. Run with `ATLAS_IMPORT_SCOPE=cyprus` only. `all` does not import Cyprus. See [Cyprus_Import.md](../phase1/cyprus/Cyprus_Import.md).
- France is Prompt AR (`country-package-france`; **35,112 current + 2,738 historical**). 37,705 municipal / 141 regional / 4 national / 0 other. Every classification stays `needs_review`. Holds G01–G21 stay open. 0 published events and 0 published result rows. Slim land omits `events.jsonl` (119,554), `results.jsonl` (1,193,657), `reporting-units.jsonl` (173,409), and `sources/`; those rows are not invented. The successor crosswalk stays empty. `FR-EP` stays `national_context`. Run with `ATLAS_IMPORT_SCOPE=france` only. `all` does not import France. See [France_Import.md](../phase1/france/France_Import.md).
- Germany is Prompt AS (`country-package-germany`; **21,960 current + 670 historical**). Numeric tiers 3 / 20 / 552 / 22,055 stay on each row. Schema interchange is 3 national / 572 regional / 22,055 municipal / 0 other. Every classification stays `needs_review`. Holds DE-G01–DE-G23 stay open. 0 published events and 0 published result rows. Slim land has no `data/research/germany/`; omitted results and events are not invented and are not given omitted-total counters. Successor edges stay 0. Run with `ATLAS_IMPORT_SCOPE=germany` only. `all` does not import Germany. See [Germany_Import.md](../phase1/germany/Germany_Import.md).
- The United Kingdom is Prompt AU (`country-package-united-kingdom`; **482 current + 2 shadow + 26 historical-only**). Numeric tiers 2 / 5 / 76 / 427 stay on each row. Schema interchange is 2 national / 81 regional / 427 municipal / 0 other. Every classification stays `needs_review`. Holds G01–G27 stay open. 0 published events and 0 published result rows. Slim land omits `events.jsonl` and `results.jsonl.gz`; those rows are not invented and are not given omitted-total counters. Successor edges stay 0. Run with `ATLAS_IMPORT_SCOPE=united_kingdom` only. `all` does not import the United Kingdom. See [United_Kingdom_Import.md](../phase1/united-kingdom/United_Kingdom_Import.md).
- Italy is Prompt AT (`country-package-italy`; **15,917 current + 696 historical + 8 pending FVG**). Numeric tiers 4 / 38 / 11 / 16,568 stay on each row. Schema interchange is 4 national / 49 regional / 16,568 municipal / 0 other. Every classification stays `needs_review`. Holds IT-G01–IT-G19 stay open. 0 published events and 0 published result rows. Slim land omits `events.jsonl` and `results.jsonl.gz`; those rows are not invented and are not given omitted-total counters. Successor edges stay 0. Pending FVG offices are not given election dates. Run with `ATLAS_IMPORT_SCOPE=italy` only. `all` does not import Italy. See [Italy_Import.md](../phase1/italy/Italy_Import.md).
...
- Romania is scoped (`ATLAS_IMPORT_SCOPE=romania`) and is proved after the `all` import: 6,460 offices / 6,372 municipal / 84 regional / 3 national / 1 other / 19,343 events / 23 result rows / 0 proceedings / holds RO-G01–RO-G07 open
- Greece is scoped (`ATLAS_IMPORT_SCOPE=greece`) and is proved after the `all` import: 703 offices / 674 municipal / 26 regional / 2 national / 1 other / 2,774 events / 3,555 proceedings / 14,004 result rows / holds GR-G01–GR-G10 and 14 local source holds open
- Luxembourg is scoped (`ATLAS_IMPORT_SCOPE=luxembourg`) and is proved after the `all` import: 130 offices / 128 municipal / 0 regional / 1 national / 1 other / 438 events / 0 result rows / 28 explicit predecessor edges / holds LU-G03–LU-G11 open
- Malta is scoped (`ATLAS_IMPORT_SCOPE=malta`) and is proved after the `all` import: 215 offices / 204 municipal / 8 regional / 2 national / 1 other / 223 events / 0 result rows / 0 successor edges / named holds stay open
- Cyprus is scoped (`ATLAS_IMPORT_SCOPE=cyprus`) and is proved after the `all` import: 888 offices / 877 municipal / 5 regional / 5 national / 1 other / 1,599 events / 0 result rows / 0 successor edges / CY-G01–CY-G15 stay open
- France is scoped (`ATLAS_IMPORT_SCOPE=france`) and is proved after the `all` import: 37,850 offices / 37,705 municipal / 141 regional / 4 national / 0 other / 0 events / 0 result rows / 0 successor edges / G01–G21 stay open
- Germany is scoped (`ATLAS_IMPORT_SCOPE=germany`) and is proved after the `all` import: 22,630 offices / numeric tiers 3 / 20 / 552 / 22,055 / schema interchange 3 national / 572 regional / 22,055 municipal / 0 other / 0 events / 0 result rows / 0 successor edges / DE-G01–DE-G23 stay open
- The United Kingdom is scoped (`ATLAS_IMPORT_SCOPE=united_kingdom`) and is proved after the `all` import: 510 offices / numeric tiers 2 / 5 / 76 / 427 / schema interchange 2 national / 81 regional / 427 municipal / 0 other / 0 events / 0 result rows / 0 successor edges / G01–G27 stay open
- Italy is scoped (`ATLAS_IMPORT_SCOPE=italy`) and is proved after the `all` import: 16,621 offices / numeric tiers 4 / 38 / 11 / 16,568 / schema interchange 4 national / 49 regional / 16,568 municipal / 0 other / 0 events / 0 result rows / 0 successor edges / IT-G01–IT-G19 stay open
