...
| `npm run import:atlas` | `scripts/atlas/import.ts` — Albania Phase 1 importer plus Andorra / Alderney / Armenia / Austria / Belgium / Bosnia and Herzegovina / Bulgaria / Netherlands and approved LatAm/NZ continuity (`ATLAS_IMPORT_SCOPE`) |
...
`data/research/denmark/`. Import with `ATLAS_IMPORT_SCOPE=denmark`
(do not use `all` on the VPS for this lineage). Slim pack omitted bulky
sources; do not invent those bytes.
...
| Denmark Prompt X field map | **Importer landed** in [docs/phase1/denmark/](phase1/denmark/README.md). Mapping Done. `ATLAS_IMPORT_SCOPE=denmark` publishes 106 current + 240 historical offices. Greenland/Faroe Realm, 2007/earlier mergers, and 293 focused-tier reviews stay open. |

Still out of scope: redirects, cutover, residual-heavy draft packs, and tightness. Austria, Belgium, Bosnia, Bulgaria, Denmark, and Netherlands `import:atlas` load their approved lineages; the observatory still skips Austria/Bulgaria XZ and Bosnia gzip. Prompt M 95 sibling withholds: disposition accepted 2026-09-17 and docs PR #27 landed; production override remains 67.
