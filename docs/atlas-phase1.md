...
| `npm run import:atlas` | `scripts/atlas/import.ts` — Andorra / Alderney / Armenia / Austria / Belgium / Bulgaria / Netherlands / Switzerland / Denmark / Sweden / Finland / Norway / Ireland / Poland / Czechia / Croatia / Portugal / Spain / Estonia and approved LatAm/NZ continuity (`ATLAS_IMPORT_SCOPE=all`). Albania Prompt BA is `ATLAS_IMPORT_SCOPE=albania` only |
...
| Albania Prompt BA field map | [docs/phase1/albania/](phase1/albania/Albania_Import.md) — mapping **Done**; importer **landed** (`ATLAS_IMPORT_SCOPE=albania` only; not part of `all`; **123 current + 768 historical**; 0 events; 0 result rows; AL-BA-G01–AL-BA-G21 open; `research_coverage_complete` stays false; Phase 1 approved tiers stay documentary) |
...
importer publishes those 891 draft offices at `needs_review` and keeps the
Phase 1 file documentary. `ATLAS_IMPORT_SCOPE=albania` only; `all` does not
import Albania. Holds AL-BA-G01–AL-BA-G21 stay open.
...
| Albania tier file | **Documentary** 2026-09-16 bytes preserved at `docs/phase1/albania/Phase1_approved_tiers.json` (122 municipal office IDs; SHA-256 `53a31d441761952a9f511c58a397e7877616c0ad6af30dce7587bcb6bcbbd93d`). Schema path `schemas/atlas/tiers/albania.json` is the Prompt BA draft (891 rows; `justin_approved` false). The live importer publishes that draft at `needs_review`. |
...
| Albania Prompt BA field map | **Importer landed** in [docs/phase1/albania/Albania_Import.md](phase1/albania/Albania_Import.md). Checked-in `albania.json` is the Prompt BA draft: 123 current + 768 historical-only (1 national / 868 municipal / 22 other; 891 rows; `needs_review`; `justin_approved` false). AL-BA-G01–AL-BA-G21 stay open. `research_coverage_complete` stays false. Phase 1 approved 122-municipal bytes stay documentary. `ATLAS_IMPORT_SCOPE=albania` only. Slim land publishes 0 events, 0 result rows, and 0 sources. |
