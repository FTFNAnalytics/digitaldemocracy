...
The accept-with-holds land kept `applied_changes=0` and did not run the importer. The follow-up importer is `ATLAS_IMPORT_SCOPE=greece` — see [Greece_Import.md](Greece_Import.md). It publishes 693 current + 10 historical offices, 2,774 events, 3,555 proceedings, and 14,004 result rows (8,021 distinct observations) and leaves GR-G01–GR-G10 plus 14 local source holds open. `ATLAS_IMPORT_SCOPE=all` does not import Greece. VPS deploy is out of scope. Slim land omits raw `sources/` (see [SOURCES_NOTE.md](SOURCES_NOTE.md)); source rows are not invented. `docs/phase1/greece/validation.json` is the pre-acceptance PASS receipt (`historical_numeric_coverage_complete=false`). Do not invent omitted source bytes, Kallikratis successor edges, or zero-filled gaps. PASS does not mean complete historic coverage.
...
- [x] Justin separately authorizes implementation (`ATLAS_IMPORT_SCOPE=greece`; not part of `all`).
