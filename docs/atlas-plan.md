...
**Phase 0** (this plan) is merged. **Phase 1 Albania storage proof has landed.** Phase 2 continuity ingest of approved packs and the `/atlas` shell (index / countries / offices) have landed; remaining residual-heavy packs, Bosnia Atlas import, and cutover have not. Austria `import:atlas` loads lineage `country-package-austria` (2,034 municipal / 4 regional). Observatory still skips the XZ payload.
