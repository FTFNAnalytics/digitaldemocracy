...
| [`montenegro.json`](montenegro.json) | 29 | `national` (2) + `municipal` (27) | **Accepted-with-holds** 2026-09-25 (Justin / Prompt AY). 27 current + 2 historical-only. Draft rows landed 1:1; every `justin_approved` stays false; `review_status` stays `draft_for_human_review`. HOLD ME-AY-G01–ME-AY-G11. Direct local executives 0. Current direct executive: President. EP offices 0. Both territorial relations keep `successor_office_id` null. `research_coverage_complete` stays false. Atlas importer waits |
| [`serbia.json`](serbia.json) | 178 | `national` (2) + `regional` (1) + `municipal` (175) | **Accepted-with-holds** 2026-09-25 (Justin / Prompt AX REBUILT). 173 current + 5 historical-only. Draft rows landed 1:1; every `justin_approved` stays false; `review_status` stays `draft_for_human_review`. HOLD RS-AX-G01–RS-AX-G11, including the Kosovo scope gate (RS-AX-G05). Kosovo-scope offices 0. Direct local executives 0. Current direct executive: President. EP offices 0. Five sourced same-territory status changes; no merger edges. `research_coverage_complete` stays false. Atlas importer waits |
| [`north-macedonia.json`](north-macedonia.json) | 172 | `municipal` (170) + `national` (2) | **Accepted-with-holds** 2026-09-25 (Justin / Prompt AZ). 164 current + 8 historical-only. Draft rows landed 1:1; every `justin_approved` stays false; `review_status` stays `draft_for_human_review`; `classification_kind` stays `research_draft`. HOLD MK-AZ-G01, MK-AZ-G04, MK-AZ-G05, MK-AZ-G09–MK-AZ-G12, MK-AZ-G14–MK-AZ-G17, MK-AZ-G19–MK-AZ-G23. Mayors and President popular. EP 0. Regional 0. Successor edges empty. `sources/`, `events.jsonl`, and `results.jsonl` omitted. `research_coverage_complete` stays false. Atlas importer waits |
...
and identity vectors. Import with `ATLAS_IMPORT_SCOPE=hungary` only. `all` does not import Hungary. Imported result rows stay 0.
...
offices, or hold resolutions. Import with `ATLAS_IMPORT_SCOPE=romania` only;
`all` does not publish Romania.
...
artifacts). Import with `ATLAS_IMPORT_SCOPE=greece` only; `all` does not publish Greece.
...
**Albania Phase 1:** all 122 office-register rows are **approved** `municipal` (Justin,
2026-09-16) for Phase 1 storage proof. Those bytes are preserved at
`docs/phase1/albania/Phase1_approved_tiers.json` (SHA-256
`53a31d441761952a9f511c58a397e7877616c0ad6af30dce7587bcb6bcbbd93d`). Regional
count on that classifier is 0 by design. The importer still classifies those
122 rows. Checking in the Phase 1 file is not a populated regional calendar.

**Albania Prompt BA** is **accepted with holds** (Justin 2026-09-25
America/Edmonton; checked-in tier SHA-256
`38534cec38c039c6807c4b2347fb46bf172ea20aa2252735ca6a6d26a362c8ae`;
pack `data/draft-tiers.jsonl`
`e30634545547376cc5271fbd7c281fec483ebc30e518276e1e78d1eec349e965`).
Exact 891 office IDs: **123 current + 768 historical-only**. Draft tiers
are **1 national / 868 municipal / 22 other**. Every row stays
`justin_approved: false` and `draft_unapproved`. Named holds stay open:
AL-BA-G01 through AL-BA-G21. `research_coverage_complete` stays false.
Current scope is the Assembly, 61 municipal councils, and 61 direct mayors.
The President is Assembly-elected; no popular president office was added.
European Parliament offices stay 0. The 22 `other` rows are the 2011 Tirana
borough offices supplied by the pack. Historical 768 rows are source-vintage
identities, not proved abolitions. `office_successor_edges` stays empty.
Do not invent omitted `sources/`, `events.jsonl`, or `results.jsonl` bytes,
offices, votes, or successor edges. This land is docs and draft tiers only.
The existing importer still publishes the preserved 122 municipal rows.
`ATLAS_IMPORT_SCOPE` is unchanged.

**North Macedonia Prompt AZ** is **accepted with holds** (Justin
2026-09-25 America/Edmonton; checked-in tier SHA-256
`3910a381b31456f4a46e52c39825012cb3b1f92c1e4c18ab501a9a713991572d`;
predecessor `docs/phase1/north-macedonia/data/draft-tiers.jsonl`
`61335b650f482f461b8545785f24dd8d404087efeca34c9d6dd681c8f4033dea`).
Exact 172 office IDs: **164 current + 8 historical-only**. Draft tiers
are **2 national / 170 municipal**. Every row stays
`justin_approved: false`, `draft_for_human_review`, and
`classification_kind: research_draft`. Named holds stay open: MK-AZ-G01,
MK-AZ-G04, MK-AZ-G05, MK-AZ-G09 through MK-AZ-G12, MK-AZ-G14 through
MK-AZ-G17, and MK-AZ-G19 through MK-AZ-G23. Pack-resolved MK-AZ-G02,
MK-AZ-G03, MK-AZ-G06, MK-AZ-G07, MK-AZ-G08, MK-AZ-G13, and MK-AZ-G18 stay
as recorded. `research_coverage_complete` stays false. Current direct
executives: 82. Current councils: 81. Historical-only offices: 8.
European Parliament offices: 0. Regional offices: 0.
`office_successor_edges` stays empty. The 2013 Kichevo claim is a
territorial reorganization, not an office identity. The 2019 rename does
not create a new office. Do not invent omitted `sources/`,
`events.jsonl`, or `results.jsonl` bytes, offices, votes, or successor
edges. This land is docs and draft tiers only. The Atlas importer waits.
Do not add North Macedonia to `ATLAS_IMPORT_SCOPE`.
...
labels come from the Prompt E pack; they are not invented here. Andorra, Alderney, Armenia, Austria, Bosnia and Herzegovina, Bulgaria, Belgium, Netherlands, Switzerland, Denmark, Sweden, Finland, Norway, Ireland, Poland, Czechia, Croatia, Portugal, Spain, Estonia, Latvia, Hungary, Romania, Greece, Luxembourg, France, Cyprus, Malta, Lithuania, Slovakia, Slovenia, Germany, the United Kingdom, and Italy files above are unchanged. Albania `albania.json` is the Prompt BA draft described above; the Phase 1 approved bytes are preserved beside it. North Macedonia Prompt AZ is the new draft above.
